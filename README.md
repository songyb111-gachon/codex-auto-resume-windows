...
**Codex Auto Resume**로 표시됩니다. 아무것도 하지 않으면 재개됩니다. **재개하지 않음**은 취소만
하고, **대시보드 열기**는 대시보드의 대기 중 페이지를 열기만 합니다. 아래 그림은 **대시보드 열기**
버튼이 생기기 전, 그리고 식별자 줄이 "Thread"로 표시되던 때에 찍은 것입니다. 지금 알림에 무엇이
적히는지는 [알림](#알림)에 있습니다.
...
바꿀 수 있는 패널이 열립니다. 일반, 복구, 알림, 이어서 하기 메시지 부분이 있고, 워처가 실제로 보낼
메시지의 미리보기와 대기 중인 대화마다의 스위치가 있습니다. 패널은 Codex의 밝은 테마·어두운 테마와
움직임 줄이기 설정을 따릅니다. 직접 입력 메시지는 패널에 보이기만 하고 고칠 수는 없으며, 고치는
...
상태, 그리고 설정을 한 창에서 보여 줍니다. 머리글의 점은 워처가 하는 일을 보여 줍니다. 감시 중(느린
숨쉬기), 기다리는 중, 때가 된 작업 확인 중, 복구 중, 일시 중지됨, 확인이 필요함(한 번 깜박임)입니다.
설정 > 모양의 **움직임 줄이기**는 이 애니메이션을 멈추며, Windows 자체의 애니메이션 설정도 항상
따릅니다. 고대비 모드에서는 그림자와 색조를 뺍니다. 네이티브 창이며, 로컬 웹 서버를 띄우거나
브라우저를 여는 일은 없습니다.

이 문서의 그림은 합성 기록을 넣은 임시 설치에서 찍은 것입니다. 창이 어떻게 보이는지를 보여 줄 뿐
...
대해 워처가 마지막으로 기록한 안전 확인 목록을 보여 줍니다. 줄마다 있는 **자동 이어 가기** 체크박스는
그 작업이 속한 대화의 자동 복구를 끄거나 켭니다. 클릭에는 그 줄을 그릴 때의 정확한 중단과 대화가
함께 실리며, 누르는 사이에 끝났거나 사라졌거나 다른 대화의 것으로 밝혀진 작업에 대한 클릭은 거절되고
아무것도 바꾸지 않습니다. 어느 쪽이든 체크박스는 아무것도 보내지 않습니다. **지금 다시 확인**은 워처에게
...
<img src="docs/images/dashboard-pending-ko.png" alt="대시보드의 대기 중 페이지. 사용량 초기화를 기다리는 대화와 재시도가 예약된 대화가 각각 상태 칩과 자동 이어 가기 체크박스와 함께 보이고, 선택한 작업의 기다리는 이유 목록, 지금 다시 확인·취소·진행 기록·이 대화는 끄기·모두 취소 버튼" width="680">
...
체크박스가 있습니다), **자동 복구 일시 정지** 또는 **자동 복구 다시 켜기**, **대시보드 열기**가 있습니다.
...
**자동 복구 다시 켜기**), **워처 중지**가 있습니다. 남은 시간이 0이 되어도 워처가 다시 살펴볼 뿐, 그것
...
| 자동 복구 | 분류된 중단 가운데 어떤 종류를 복구할지. 종류마다 스위치가 하나씩 있습니다 |
| 이어서 하기 메시지 | Codex에 보내는 메시지의 언어와 스타일, 직접 입력 메시지, 실제 문장 미리보기 |
| 모양 | 테마, 움직임 줄이기 |
| 고급 | 중단당 시도 횟수, 진전 없는 복구 뒤 멈출 시점, 작업당 이어서 하기 횟수, 재시도 간격 |

워처가 복구하는 종류에는 모두 스위치가 있고 기본으로 켜져 있습니다. 로그인 서비스를 일시적으로 쓸 수
없는 경우(`auth_service_transient`)는 v0.6.2까지 스위치 없이 복구되었고, 지금은 **로그인 서비스 오류**
스위치가 있습니다.
...
워처가 중단을 기록하면 Windows가 작업 이름이 적힌 알림 하나를 보여 줍니다.
...
알림 영역 팝업의 작업마다 붙은 체크박스와, 대시보드 대기 중 페이지의 **자동 이어 가기** 열입니다.
이 체크박스는 그 중단 하나를 취소하는 것이 아니라, 그 작업이 속한 대화의 자동 복구를 다시 켤 때까지
...
언어, 메시지 스타일(직접 입력으로 바꾸는 것까지)을 바꿀 수 있지만 직접 입력 문장은 쓸 수 없습니다. 그
...
명령줄 사용법과 아키텍처, 테스트 실행 방법은 영어 [README](https://github.com/songyb111-gachon/codex-auto-resume-windows/blob/main/README.md)에만 있습니다.
