...
대시보드와 팝업과 그 메뉴를 맞춰 그리기 위한 고대비 사용 여부와, 이번 릴리스부터는 Windows가 앱을 밝게 또는 어둡게 표시하도록 설정되어 있는지(읽기만 하고 쓰지 않는
...
- `get_status`: 버전, 복구가 켜져 있는지, 워처가 실행 중인지, 로그인 자동 시작이 등록되어 있는지, 상태별
  ...
  직접 쓴 직접 입력 메시지의 문장도 들어 있습니다(Codex는 거기서 그것을 읽을 수 있을 뿐 바꿀 수는 없습니다). 이번 릴리스부터는
