...
- `get_status`: 버전, 복구가 켜져 있는지, 워처가 실행 중인지, 로그인 자동 시작이 등록되어 있는지, 상태별
  개수, 사용자 설정. 설정에는 Codex 실행 파일 경로를 직접 지정했다면 그 경로도 들어 있습니다. v0.5.0부터
  현재 최신 릴리스까지는 설치 디렉터리 경로(보통 Windows 사용자 이름이 들어 있습니다)도 돌려줍니다.
  그것을 빼는 변경은 main 브랜치에 있으며 다음 릴리스에 포함됩니다;
...
- `config/state.sqlite` — 대기 중인 복구: 대화 UUID, 중단 id, 실패한 턴의 id와 번호, 타임스탬프, 장애 분류,
  ...
  스위치(켠 시각과 폴링 간격 포함)와 대화별 스위치도 여기에 있습니다. 어느 것도 내용이 아닙니다;
