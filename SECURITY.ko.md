...
- **호환성 데이터는 제한만 할 수 있습니다.** 이 항목은 v0.6.4 다음 릴리스에 포함됩니다. Codex 호환성
  ...
  만들었습니다. 로컬 확인의 실패가 언제나 우선합니다. 문서가 무엇을 말하든 실패는 호환되지 않음 말고는 될 수
  없습니다. 문서는 정확한 Codex 버전이나 버전 범위에 대해 기능을 호환되지 않음으로 표시할 수 있고, 그러면 그
  데이터가 적용되는 동안 워처는 아무것도 보내지 않습니다. 검증됨으로 표시하는 것은 정확한 버전에 대해서만,
  증거 파일을 인용해서만, 그리고 로컬 확인이 이미 통과한 곳에서만 할 수 있으며, 보이는 단어를 바꿀 뿐 무엇을
  보내는지는 바꾸지 않습니다. 실행할 수 없었던 확인은 데이터가 무엇을 주장하든 알 수 없음으로 남고, 알 수
