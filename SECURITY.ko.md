...
이번 수락 감사에서 검사 대상 게시본의 전송 전 투영·허용 상태 확인, 일일 시도 횟수 계산,
설치 실패 처리의 결함을 재현했습니다. `CHANGELOG.ko.md`의 미배포 수정은 소스 변경뿐입니다.
게시본은 바꾸지 않았으며 실제 수락 검사 전체를 통과하지 않았습니다. 아래 안전장치는 이
한계를 함께 고려해 읽어야 합니다.
