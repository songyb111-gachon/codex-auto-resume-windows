...
고치므로, Codex는 사용자가 설정한 모든 Git 마켓플레이스를 새로 고칩니다. v0.5.7 다음 릴리스에서는 이 제품 자신의 마켓플레이스를 이름으로 지정해 그 하나만 새로
...
- **복구 런타임에는 네트워크 코드가 없음.** `src/`와 `scripts/*.py`의 어떤 파일도 네트워킹 모듈을
  ...
  - 이 도구가 시작하는 Codex 프로세스는 복구할 차례가 된 건이 있을 때 OpenAI에 현재 사용량을
    ...
    OpenAI에 알립니다. v0.5.7에서는 `codex_auto_resume`와 `0.1`이고, v0.5.7 다음 릴리스에서는
  ...
  - 설치: `scripts/bootstrap.ps1`이 HTTPS로 GitHub에서 릴리스를 내려받고, 그 안의 무엇이든 실행되기 전에
    ...
    호스트들에서 내려받습니다. v0.5.7 다음 릴리스에서는 이 제품 자신의 마켓플레이스를 이름으로 지정하며, 이는
  ...
  - 대화 안에서 이 플러그인의 tool을 쓰거나 Codex에게 이 도구의 명령을 실행하게 하면, 그 결과(상태, 대화
    ...
    들어간 설치 경로도 들어갑니다. v0.5.7 다음 릴리스에서는 `get_status`에 설치 경로가 더 이상 들어가지 않으며,
    이는 이번 릴리스에 들어 있습니다.
...
- **복구를 다시 켜거나 늘리는 tool에는 MCP 주석으로 승인을 요청합니다.** 이 항목은 `main` 브랜치에
...
이후의 검토에서 발견해 `main` 브랜치에서 고친 것으로, 이번 릴리스에 들어 있습니다(v0.5.7에는 아래 문제가 모두 남아 있습니다):
...
가리키는 내용은 없습니다. - 대화 속 내용이 확인 창 없이 복구를 일시 중지할 수 있습니다. v0.5.7에서는 `set_auto_recovery`로, `main`
