...
- v0.6.8 다음 릴리스부터, **Codex가 시작될 때 함께 시작**이 켜져 있으면 워처 프로세스 자체. MCP 서버가
  Windows 로그인 때와 같은 실행기를 시작하며, 워처가 돌고 있지 않고 설치가 잠금을 쥐고 있지 않으며 설치본의
  실행기가 있을 때만 그렇게 합니다. 사람이 대시보드에서 정해 두는 선택이지 모델이 부를 수 있는 tool이
  아니므로(`update_settings`는 이 설정을 거절합니다) 그 사이에 승인 요청이 끼어들 일도 없습니다. 워처를
  시작한다고 해서 보낼 수 있는 것이 달라지지는 않습니다. 그것은 아래의 관문들이 정합니다.
...
- **호환성 데이터는 제한만 할 수 있습니다.** 이 항목은 v0.6.4 다음 릴리스에 포함됩니다. Codex 호환성
  ...
  데이터가 적용되는 동안 워처는 아무것도 보내지 않습니다. 검증됨으로, 그리고 이번 릴리스부터는 점검됨으로
...
지금까지 게시된 모든 압축 파일, 즉 v0.5.0부터 v0.5.7까지는 예전의 단일 작업 워크플로가 빌드했습니다. 그 워크플로는 Action을 떠다니는 태그로
...
확인하기*). - 지금까지 게시된 모든 압축 파일, 즉 v0.5.0부터 v0.5.7까지는 떠다니는 태그로 참조한 Action을 쓰는 예전의 단일 작업 워크플로가
