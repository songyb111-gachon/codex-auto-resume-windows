...
- **화면은 요청만 하고, 보내는 것은 워처뿐입니다.** 이 항목은 v0.6.2 다음 릴리스에 포함됩니다. 대기 중
  페이지와 알림 영역 팝업에서 작업마다 있는 **자동 이어 가기** 체크박스는 그 행을 그릴 때의 정확한 중단
  id와 대화 id를 함께 싣습니다. 클릭이 도착하면 제어 계층이 기록을 다시 읽어, 기록이 더 이상 없거나 이미
  끝났거나 다른 대화의 것이면 거부하고, 거부된 클릭은 아무것도 바꾸지 않습니다. 그래서 그리는 시점과 누르는
  시점 사이에 목록 순서가 바뀌어도, 한 작업에 대한 클릭이 다른 작업을 바꾸는 일은 없습니다. 이 체크박스가
  바꾸는 것은 그 대화의 스위치, 곧 정책입니다. 아무것도 보내지 않으며, 켜더라도 모든 확인을 그대로 거칩니다.
