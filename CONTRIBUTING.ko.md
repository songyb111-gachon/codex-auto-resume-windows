...
모아 만듭니다. 그래서 코드를 한 모듈에서 다른 모듈로 옮기기만 해서는 매니페스트가 바뀌지 않고, 그림에
...
적어 두는데, 각 예외는 사라지는 순간 테스트를 실패시키므로 그 목록은 줄어들기만 합니다. 같은 파일에는
함수 안에서 하는 import가 이유와 함께 모두 적혀 있어, 새로 하나를 더하면 거기에도 한 줄이 필요합니다.
`tests/test_sizes.py`는 모든 모듈에 700줄의 예산을 주고, 이미 그보다 긴 모듈은 지금 길이를 넘지 못하게
합니다.
...
소스를 읽습니다. `src/` 아래에서 추적되는 모든 `.py` 파일을, 깊이와 상관없이 읽습니다. 무언가가 없다는
