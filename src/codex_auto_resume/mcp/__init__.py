...
stayed where it is and re-exports what moved. `panel.py` is still beside it too; it becomes
`panel.py` here with the renames that end step 13.

`assets/panel.css` and `assets/panel.js` were 2,174 of `panel.py`'s 2,332 lines, written as two
...
panel to something still reads it through `panel._STYLE` and `panel._SCRIPT`, which have not
moved.
"""
