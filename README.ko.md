...
사용량 한도 안내에 왜 체크박스를 넣지 않는지는 [docs/PLUGIN.md](docs/PLUGIN.md)에 있습니다.
...
- 대화는 **정확한 UUID**로만 식별합니다. `--last`도, 제목도, 프로젝트도, 작업 디렉터리도, 최근 순도
  쓰지 않습니다.
- 분류하지 못한 장애는 절대 재시도하지 않습니다.
- 사용자가 취소한 경우, 권한·승인·콘텐츠 정책 거부, 잘못된 요청, 컨텍스트 길이 초과, 영구 인증
  실패는 절대 재시도하지 않습니다.
- 결과가 불확실한 전송은 자동으로 다시 보내지 않습니다. 그런 기록은 종료 상태에서 멈춥니다.
- 모든 관문은 전송 직전에, 디스패치 락 안에서 다시 확인합니다.
- 설정은 정책일 뿐입니다. 설정 스키마의 어떤 값도 안전 한계에 닿을 수 없고, 설정 파일이 망가졌을 때
...
잃지 않습니다. 전부 지우려면 `Uninstall.cmd -Purge`를 실행하거나(소스에서는
`uninstall --purge`) `%USERPROFILE%\.codex-auto-resume\` 폴더를 직접 지우면 됩니다.

제거는 의도적으로 보수적입니다.

- 이 설치본이 자기 것임을 증명하지 못하는 디렉터리에서는 아무것도 지우지 않습니다. 루트에 provenance
  marker가 있거나, `config/`에 있거나, `runtime.json`이 바로 그 디렉터리를 가리켜야 합니다. 단지
  `app`, `runtime`, `config`, `logs`라는 이름의 폴더가 들어 있을 뿐인 디렉터리는 거부하고 그 사실을
  알립니다.
- 소유한 디렉터리 안에서도 자기 파일 이름만 지우므로 관계없는 파일은 남습니다.
- 로그인 자동 시작 값, 알림 식별자, 시작 메뉴 항목은 여전히 **이** 설치본에 속할 때만 제거합니다.
  다른 사본을 시작하는 값은 지우지 않고 알립니다.
- 워처가 실행 중이거나 **실행 여부를 확인할 수 없으면** 아무것도 지우지 않고 중단합니다.

Codex 대화, Codex 파일, 사용자 저장소, 상위 디렉터리는 어떤 경우에도 건드리지 않습니다.
...
- 개발 과정: [docs/DEVELOPMENT.ko.md](docs/DEVELOPMENT.ko.md)
- 같은 영역의 다른 프로젝트들과의 비교: [docs/COMPARISON.ko.md](docs/COMPARISON.ko.md)
- 기여자: [CONTRIBUTORS.ko.md](CONTRIBUTORS.ko.md)

아직 영어만 있는 문서: 플러그인 구조와 사용량 한도 안내에 체크박스를 넣지 못하는 이유는
[docs/PLUGIN.md](docs/PLUGIN.md), 명령줄 사용법과 아키텍처는 영어 [README](README.md),
