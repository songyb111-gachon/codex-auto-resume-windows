...
| **개인정보** | 텔레메트리 없음, 분석 없음, 자동 업데이트 확인 없음, 자격 증명은 읽지 않음. 창의 *업데이트 확인*은 눌렀을 때만 GitHub에 가장 최근 릴리스를 묻습니다. watcher에는 네트워크 코드가 없으며, 사용량 확인과 재개된 턴, 그리고 대화 안에서 이 플러그인의 도구와 명령이 돌려준 결과는 여느 Codex 통신처럼 Codex를 통해 OpenAI로 갑니다. 설치 스크립트는 GitHub에서 릴리스를 내려받고, v0.5.7의 설치기는 Codex가 사용자가 설정해 둔 모든 Git 마켓플레이스를 새로 고치게 합니다(이 마켓플레이스만 지정하는 동작은 이번 릴리스부터입니다) |
...
2. **압축을 풀기 전에 확인합니다.** `Install.cmd`는 자신이 들어 있던 압축 파일을 검증하지
   ...
   정확한 커밋에 고정한 action, 재현 가능한 실행 파일은 게시된 v0.5.7 다음 릴리스에 포함됩니다.
...
않습니다. Codex 플러그인도 압축 파일 안의 파일로 등록하므로 이 경로에서도 패널까지 함께 설치됩니다. 다만 Codex에 마켓플레이스 새로 고침은 요청합니다. v0.5.7과 그 이전 설치기는 모든 Git 마켓플레이스의 새로 고침을 요청합니다. 현재 v0.5.7 다음 릴리스 설치기는 이 제품의 마켓플레이스만 지정하므로, 방금 등록한 로컬 마켓플레이스에
...
시작 메뉴에서 여는 독립 창은 Codex를 꺼 둔 상태에서도 동작합니다. v0.5.7 다음 릴리스의 이 창은 대시보드입니다. 워처가 무엇을 하고 있는지, 무엇이 대기 중이고
언제 다시 확인하는지, 무엇이 어떻게 끝났는지, 최근 일주일 수치, 워처 상태, 그리고 설정을 한 창에서
보여 줍니다. 네이티브 창이며, 로컬 웹 서버를 띄우거나 브라우저를 여는 일은 없습니다. 아래 그림은
합성 기록을 넣은 임시 설치에서 이번 수락 검증 전에 찍은 것입니다. 게시된 릴리스를 설명하는 그림이며,
실제 Codex에서 복구를 관찰했다는 증거는 아닙니다. 아직 게시하지 않은 수정 사항에 맞춰 다시 촬영하지 않았습니다.
...
- **GitHub, 설치할 때.** 플러그인으로 설치하거나 업데이트하면 설치 스크립트가 GitHub에서 해당
  ...
  항목).
- **설치기가 실행될 때의 마켓플레이스 호스트.** v0.5.7 다음 릴리스은 `codex-auto-resume-windows`만 지정합니다.
