...
빛나지 않는 회색이고, 확인이 필요하면 주황색으로 한 번 깜빡인 뒤 켜져 있습니다. 옆의 단어가 늘 어느
상태인지 알려 줍니다.
...
동안에는 숨 쉬지 않고 두 배 빠르게, 3.96초마다 한 번씩 나갔다 돌아오기를 되풀이합니다. 일시 정지되면 회색이고 멈춰 있고, 사람이 봐야 할 일이 생기면 주황색으로 바뀌어 한 번
맥박친 뒤 머물며, 실패하면 빨간색으로 바뀌어 2.2초마다, 다른 어떤 상태보다 조금 빠르게 계속 숨 쉽니다. 움직임 줄이기, Windows의 애니메이션 설정, 고대비, 배터리 절약 모드에서, 세션이 잠겨 있는
...
팝업, Codex 안의 패널, 알림 카드가 모두 같은 불빛입니다. 복구가 진행 중이면 2.8초에, 실패하면 2.2초에
한 주기로 숨 쉬고, 대기 중과 확인 중에는 켜진 채 멈춰 있으며, 움직임 줄이기와 Windows의 애니메이션 설정,
고대비에서도 멈춥니다.

<img src="docs/images/icon-motion.png" alt="알림 영역 아이콘의 움직임을 아이콘 자신의 프레임으로 그린 모습(밝은 작업 표시줄). 왼쪽부터 감시 중(밝은 머리가 숨 쉰 뒤 가장 밝은 채로 고리의 흰 획을 따라 시계 방향으로 쓸고 갔다가 돌아옵니다), 복구 중(계속 나갔다 돌아옵니다), 확인 필요(주황색으로 한 번 맥박친 뒤 멈춥니다), 실패(빨간색으로 2.2초마다 숨 쉽니다), 일시 정지(회색으로 멈춰 있습니다)" width="360">
