...
배지의 짙은 파랑 쪽으로 흐려졌다가 돌아오며 숨 쉬고, 네 번 숨 쉰 뒤에는 가장 밝은 채로 고리를 따라 시계
방향으로 한 바퀴 돕니다. 도는 동안에는 숨 쉬지 않습니다. 복구가 진행되는 동안에는 숨 쉬지 않고 1.6초에 한
바퀴씩 계속 돕니다. 일시 정지되면 회색이고 멈춰 있고, 사람이 봐야 할 일이 생기면 그 색으로 바뀌어 한 번
...
<img src="docs/images/icon-motion.gif" alt="알림 영역 아이콘의 움직임을 아이콘 자신의 프레임으로 그린 모습. 위는 밝은 작업 표시줄, 아래는 어두운 작업 표시줄입니다. 왼쪽부터 감시 중(밝은 머리가 숨 쉰 뒤 가장 밝은 채로 고리를 따라 시계 방향으로 한 바퀴 돕니다), 복구 중(청록색 배지 아래에서 계속 돕니다), 확인 필요(주황색 배지와 함께 주황색으로 한 번 맥박친 뒤 멈춥니다), 일시 정지(회색 배지와 함께 회색으로 멈춰 있습니다)" width="288">
