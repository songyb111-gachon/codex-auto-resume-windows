...
두 배 빠르게, 1.98초마다 한 번씩 되풀이하고, 사용자가 볼 때까지 빨간 채로 있습니다. 아이콘을 클릭해 팝업을
...
<img src="docs/images/status-light.png" alt="카드 위에서 숨 쉬는 상태 불빛. 청록색 점이 4.4초에 걸쳐 제 색의 3분의 2 정도까지 어두워졌다가 돌아오고, 밝아질수록 점 아래에서 부드러운 빛이 번져 나왔다가 호흡의 바닥에서는 사라집니다" width="112">
...
<img src="docs/images/icon-motion.png" alt="알림 영역 아이콘의 움직임을 아이콘 자신의 프레임으로 그린 모습(밝은 작업 표시줄). 왼쪽부터 감시 중(밝은 머리가 숨 쉰 뒤 가장 밝은 채로 고리의 흰 획을 따라 시계 방향으로 쓸고 갔다가 돌아옵니다), 복구 중(계속 나갔다 돌아옵니다), 확인 필요(주황색으로 제자리에서 천천히 숨 쉽니다), 실패(빨간색으로 복구 중보다 두 배 빠르게 나갔다 돌아옵니다), 일시 정지(회색으로 멈춰 있습니다)" width="360">
...
- 엔진은 자동으로 찾으며, 어떤 버전도 버전 문자열만으로 신뢰하지 않습니다. Codex 업데이트 전이든
  ...
  실제 복구가 확인했으면 *검증됨*, 이번 릴리스부터는 관리자 자신의 점검이 그 정확한 버전에서 통과했고
