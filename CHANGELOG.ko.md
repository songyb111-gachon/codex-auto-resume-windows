...
## v0.6.4 — 하나의 모습, 차분해진 불빛, 곧바로 열리는 창

디자인과 속도에 관한 릴리스입니다. 복구 자체가 내리는 결정은 v0.6.3과 똑같습니다. 같은 분류기,
같은 안전 확인, 그리고 보낼 수 있는 유일한 구성 요소인 같은 워처 하나입니다.

### 하나의 디자인

- 대시보드와 알림 영역 팝업이 Codex 안의 패널과 같은 재질을 씁니다. 떠 있는 카드와 안으로 들어간
  입력 칸, 부드러운 그림자, 모서리 둥글기, 컨트롤이 모두 같습니다. 체크박스는 스위치가 되었고, 상태
  칩에는 테두리가 없으며, 버튼은 도드라져 있다가 누르면 들어갑니다. 글자 크기, 대시보드의 페이지와
  설정 부분, 팝업의 구성은 그대로이고, 알림 영역 아이콘과 그 배지도 그대로입니다.
...
- 가장 긴 설정 부분에 맞춰 커지는 대신 v0.6.2의 크기인 860 × 600으로 열립니다. 창보다 긴 설정 부분은
  스크롤됩니다.
