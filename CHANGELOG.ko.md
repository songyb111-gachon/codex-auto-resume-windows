...
- 워처가 감시하는 동안 표시 안의 밝은 머리가 상태 불빛과 같은 3.2초 리듬으로 숨 쉬고(배지의 짙은 파랑
  쪽으로 흐려졌다가 돌아옵니다), 30초쯤마다 한 번씩 고리를 따라 천천히 한 바퀴 돕니다.
- 복구가 진행되는 동안, 즉 continuation을 보내거나 그것이 시작한 턴을 계속 따라가는 동안에는 머리가
  1.6초에 한 바퀴씩 고리를 따라 계속 돌고, 더 빠른 숨을 쉽니다.
...
- 움직임 줄이기, Windows의 애니메이션 설정, 고대비, 배터리 절약 모드에서, 세션이 잠겨 있는 동안, 그리고
  아이콘이 아무도 보지 않는 넘침 영역에 있는 동안에는 움직이지 않습니다. 쉬고 있을 때는 늘 그랬던 그
  아이콘 그대로이고, 배지와 모양도 그대로입니다.
- 설정 창이 열려 있는 동안에는 작업 표시줄 단추도 같은 상태, 같은 리듬, 같은 프레임으로 똑같이
  움직이고, 같은 설정에서, 그리고 창이 숨겨져 있을 때 멈춥니다. 제목 표시줄의 아이콘은 움직이지
  않고, 쉬고 있을 때 단추는 늘 그랬던 그 아이콘입니다.
