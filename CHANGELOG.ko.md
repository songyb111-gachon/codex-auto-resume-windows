...
- 로드맵: **v0.6.9**는 고급 기능입니다. 비슷한 어떤 프로그램이 제공하는 것보다도 많이 제공합니다. 지금까지의
  기능은 지금의 제약을 그대로 지키고, 그 제약에서는 만들 수 없었던 기능은 모두 만들되 사용자가 켜기 전까지
  꺼져 있습니다. 이 기능들은 기본판과 함께 나오는 고급판에만 들어가고, 기본판에는 그 코드가 아예 없습니다. 프리 릴리스인 **v0.6.10-alpha**가 Python 모듈화이고, 저장소 전체의 버그 찾기를 마친 최종
  **v0.6.10**이 마지막 Python 릴리스입니다. **v0.6.11-alpha**는 코어를 그대로 Rust로 바꾸고, 최종
  **v0.6.11**은 그것을 Rust답게 만들며, **v0.6.12**는 Rust 버그 찾기입니다.
