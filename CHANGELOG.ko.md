...
- **화면 언어 설정**(설정 > 일반). 기본값인 *시스템*은 이전처럼 Windows가 첫 번째로 꼽는
  ...
  재시작·복구·업데이트 뒤에도 유지됩니다. `CODEX_AUTO_RESUME_LANG`은 여전히 둘 다보다
  우선합니다.
...
- 직접 입력 문장은 Windows 대시보드에서만 씁니다. Codex에서는 `update_settings`로도, 새로
  ...
  보내지는 문장은 모델이 설득당해 바꿀 수 있는 것이어서는 안 되기 때문입니다.
...
(실제로 실행한 것으로 채울 것: 테스트, CI, Windows 확인, Codex 확인)
