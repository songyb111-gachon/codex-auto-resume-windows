...
이 릴리스는 두 번 잘랐습니다. 첫 후보는 2026-09-20에 빌드되어 `v0.6.6`으로 태그되고 게시되었다가,
알리기 전에 멈췄습니다. 상태등이 아직 맞지 않았고, 아래의 컨트롤들이 제품의 것이 되기 전이었기
때문입니다. 지우는 대신 남겨 두었고, 이제 자기 태그를 갖습니다.
**[`v0.6.6-alpha`](https://github.com/songyb111-gachon/codex-auto-resume-windows/releases/tag/v0.6.6-alpha)**,
`f63d904`에서 빌드한 것입니다.

이 릴리스가 아니며 아무에게도 제공되지 않습니다. 업데이트 확인은 `releases/latest`가 끝나는 URL에서
태그를 읽어 `vMAJOR.MINOR.PATCH` 형식만 받아들이므로, 접미사가 붙은 태그는 이름 그대로 거절됩니다.
찾아보실 때 알아 두면 좋은 것이 하나 있습니다. **그 압축 파일의 이름은 여전히
`CodexAutoResume-v0.6.6-win-x64.zip`입니다.** 태그 이름을 바꾸어도 릴리스에 이미 붙어 있는 파일의
이름은 바뀌지 않기 때문입니다. 이름은 같고 바이트도 다이제스트도 다릅니다. `scripts/release.json`에
고정된 값은 이 릴리스의 것이고, 대조하실 값도 그것입니다.

같은 일이 한 번 더, 다른 이유로 있었습니다. 2026-09-20에 `v0.6.6`으로 게시된 빌드 - 정석대로의 숨쉬기,
제품 자신의 스크롤 막대와 대화창, 그림의 가장자리를 담은 것 - 는 같은 날 요청된 패널 자신의 테마 없이
나갔습니다. 그래서 이 릴리스를 세 번째로 잘랐고, 그 빌드는 이제
**[`v0.6.6-beta`](https://github.com/songyb111-gachon/codex-auto-resume-windows/releases/tag/v0.6.6-beta)**를
달고 프리 릴리스로 표시됩니다. 그것은 알려진 적이 있으므로 여기 분명히 적습니다. **`v0.6.6`은 두 개의
압축 파일을 가리킨 적이 있습니다.** `scripts/release.json`에 고정된 값은 이 릴리스의 것이고, 베타의 값은
그 옆 자기 릴리스 페이지에 적혀 있습니다. 거기서 받은 파일은 비교에서 어긋나며, 그것이 비교가 제 일을
하는 모습입니다.

### 확인한 것

아래는 Windows 11 기계 한 대에서 실행한 것입니다(Home, 빌드 26200, 3840 x 2160, 150% 배율, 한국어
시스템 언어, Windows 앱 모드는 어둡게, 제품의 테마 설정은 밝게). 테스트, 빌드, 불빛, 그림, 대화창은 이
나무에서 쟀습니다. 설치가 있어야 하는 세 가지 — 설치된 자리에서의 작업 표시줄 단추, v0.6.5 위 업그레이드,
아카이브 자체 점검 — 은 이 릴리스의 첫 후보에서 쟀고(아래 컨트롤들이 제품의 것이 되기 전입니다), 실제로
게시하는 후보에서 다시 잽니다.

- **테스트.** 2,572개, 실패 없음. 여기에서 Python 3.13(910초, 8개 건너뜀), 그리고 GitHub의 Windows
  러너에서 3.12, 3.13, 3.14, 3.15 모두 초록불이었습니다. 건너뛴 것은 직접 켜야 하는 실사용 점검과
  태그를 기다리는 고정 검사입니다.
- **창을 두 번 빌드.** 같은 소스에서 만든 `CodexAutoResumeSettings.exe` 두 개가 바이트까지 같았습니다
  (`c572844e…`, 387,072바이트). 아카이브 자신의 digest는 여기에 적지 않습니다. 이 파일이 아카이브 안에
...
- **v0.6.5 위로 설치.** 설치 프로그램이 *업데이트했습니다. 설정과 대기 중인 복구는 그대로 있습니다.*
  라고 답했고, `config\settings.json`은 이전과 바이트까지 같았습니다. 워처가 다시 시작해 0.6.6을
  보고했고 플러그인 매니페스트도 0.6.6을 가리킵니다.
- **아카이브.** `build/smoke_archive.py`의 모든 검사를 통과했습니다. 두 실행 파일이 0.6.6을 보고하는
...
빌드가 실제 Codex 중단을 복구하는 것, 이 기계가 아닌 다른 기계, 150%가 아닌 다른 배율.
...
- **아이콘의 움직임.** 이 기계에서는 알림 영역 아이콘이 Windows 11의 숨겨진 아이콘 영역에 있고, 이번
  릴리스는 그곳에서 일부러 멈춰 둡니다. 워처가 처음 40분 동안 쓴 처리 시간은 1.1초였습니다. 움직임 자체는
  창을 별도 폴더에 빌드해 실제 작업 표시줄 단추에서 찍어 확인했고, 테스트가 아이콘 자신의 표와 프레임
  하나하나를 맞춰 고정합니다.
