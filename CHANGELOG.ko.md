...
v0.5.5가 이미 게시된 뒤에 수행한 적대적 검토에서 나온 것들입니다. 안전이나 데이터에 관한 문제는
하나도 없으므로 v0.5.5는 게시된 그대로 두고, 여기 있는 것들은 v0.5.6이 아니라 v0.6.0으로
나갑니다.

- **고침: 워처가 어떻게 되었든 설치 프로그램은 여전히 "Installed and running."으로
  끝났습니다.** v0.5.5는 엔진이 단언하기 전에 확인하도록 만들었지만 셸 래퍼는 그대로 단언하게
  두었습니다 - 같은 과장이 한 겹 바깥에, 그것도 대부분의 사용자가 실제로 읽는 문장에 남아
  있었던 것입니다. 이제 설치는 "할 일은 모두 했지만 워처가 실행 중인 것은 확인하지 못함"에 대해
  세 번째 종료 코드를 반환합니다. 이것은 성공도 실패도 아닙니다. 실패로 처리하면 멀쩡한 설치를
  되돌리게 되고, 성공으로 처리한 것이 바로 그 문장이 살아남은 이유입니다. `enable`도 같은
  모양이었습니다 - 답을 최대 6초까지 기다린 다음, 그 답과 무관하게 "Auto resume is on."을
  출력했습니다.
- **고침: 스크린샷 최신성 검사가 읽기 경로 전체를 놓쳤습니다.** 설정 창은 자기 텍스트를 아무것도
  그리지 않습니다. 모든 레이블과 상태 줄 전체가 브리지를 건너서 오므로 `controlcli.py`와
  `app.py`가 렌더 입력이고, 아이콘과 DPI 매니페스트를 아예 컴파일에 포함할지 말지를 정하는
  `make_gui.ps1`도 마찬가지입니다.
- **고침: 한국어 sync가 코드 안까지 고쳤습니다.** 레이블 치환이 링크 문법 없는 맨 `[...]`였던
  탓에, 한국어 파일 이름을 *언급하기만* 한 fenced 블록과 인라인 코드까지 편집했습니다 - 게다가
  이를 막아야 할 테스트는 어떤 패턴에도 걸리지 않는 이름으로 fence를 만들어서, 어느 쪽이든
  통과했습니다.
- **고침: 링크가 맞는 파일의 엉뚱한 위치에 도착할 수 있었습니다.** 생성된 트리는 죽은 경로는
  검사했지만 죽은 앵커는 검사하지 않았는데, 이 sync가 실제로 무효화하는 쪽은 후자입니다. 영어
  문서를 한국어 문서로 갈아 끼우면 경로는 전부 그대로 유효하고 그 문서로 들어가는 앵커는 전부
  죽습니다. 실제로 살아 있는 사례가 `SUPPORT.md`에 하나 있었고, 이제 main의 영어 사본을
  가리킵니다.
- **고침: 로컬에서 `ko_sync.py --root .`를 한 번 잘못 돌리면 main의 한국어 불변 조건이 전부
  조용히 해제될 수 있었습니다.** 그 명령이 남기는 marker가 해당 테스트들의 유일한 스위치였고,
  `git restore`로도 사라지지 않았습니다. 이제 그 파일은 무시 대상이며, 스위치는 marker가
  *추적되고 있을* 것을 요구합니다 - 생성된 브랜치에서는 참이고, 로컬 잔여물에서는 결코 참이
  아닙니다.
- 작은 것 두 가지: 생성기가 git 출력을 공백으로 잘라 Markdown 목록을 만드는 바람에, 공백이나
  비 ASCII 문자가 든 경로가 조용히 빠졌습니다. 그리고 릴리스 매니페스트 테스트는 "게시되었지만
  고정된 적 없음"을 잡는다고 했지만, 실제로 볼 수 있는 것은 이번 릴리스에서 더 이상 쓰지 않게
  된 `null` placeholder뿐이었습니다. 이제 변경 이력의 헤딩 자체를 읽고, 다이제스트가 없으면
  실패합니다.
