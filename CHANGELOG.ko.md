...
- v0.6.4의 불빛은 너무 조용해서 잘 보이지 않았습니다. 이제 워처가 감시하는 동안 불빛의 빛무리가 거의
  없는 상태에서 뚜렷하게 켜진 상태까지 3.2초마다 오르내리고(불투명도 0.12에서 0.58), 밝아지면서 닿는
  거리의 82%에서 108%까지 커지므로, 가장 밝을 때는 점을 둘러싼 부드러운 고리로 보입니다. 복구를 보내는
  동안에는 더 빨리 숨 쉽니다. 2초마다 0.20에서 0.70입니다.
- 기다리는 중과 확인하는 중에는 0.30에 머물고, 문제가 생기면 1.4초 동안 0.72까지 한 번 맥박친 뒤
  머뭅니다. 색, 불빛 옆의 단어, 크기는 그대로이고, 대시보드와 팝업과 패널에서 같은 불빛입니다.
- 움직임 줄이기와 Windows의 애니메이션 설정에서는 여전히 멈춰 있고, 고대비에서는 여전히 빛무리 없는
  단색 점으로 그립니다.

### 알림 영역 아이콘이 움직입니다

- 워처가 감시하는 동안 표시 안의 밝은 머리가 상태 불빛과 같은 3.2초 리듬으로 네 번 숨 쉬고(배지의 짙은
  파랑 쪽으로 흐려졌다가 돌아옵니다), 그다음 숨 한 번의 시간 동안 가장 밝은 채로 고리를 따라 시계 방향으로
  한 바퀴 돕니다. 3.2초짜리 칸 다섯 개, 모두 16초가 한 주기입니다. 도는 동안에는 숨 쉬지 않고, 숨과 회전은
  모두 가장 밝을 때 시작하고 끝나므로 하나가 다른 하나로 넘어가는 자리에서 튀는 것이 없습니다.
- 복구가 진행되는 동안, 즉 continuation을 보내거나 그것이 시작한 턴을 계속 따라가는 동안에는 머리가 가장
  밝은 채로, 숨 쉬지 않고, 1.6초에 한 바퀴씩 시계 방향으로 계속 돕니다.
- 일시 정지되면 머리는 회색이고 멈춰 있습니다. 사람이 봐야 할 일이 생기면 주황색으로(실패라면 위험을
  뜻하는 색으로) 바뀌어 한 번 맥박친 뒤 그대로 머뭅니다.
- 머리는 15도씩 24단계로 돌고, 저장한 프레임이 아니라 머리에 입히는 색조인 밝기 24단계로 숨 쉽니다. 도는
  동안에는 1.6초짜리 한 바퀴의 단계마다 하나씩 초당 16프레임쯤, 숨 쉬는 동안에는 6프레임쯤입니다.
  ...
  자주 다시 그려지지 않았습니다. 복구가 진행되는 동안처럼 계속 돌 때는 각각 2.5에서 3%포인트쯤입니다.
- 움직임 줄이기, Windows의 애니메이션 설정, 고대비, 배터리 절약 모드에서, 세션이 잠겨 있는 동안, 그리고
  아이콘이 아무도 보지 않는 넘침 영역에 있는 동안에는 움직이지 않습니다. 다만 넘침 영역은 Windows가 그렇다고
  알려 줄 때만 알 수 있는데, Windows 11 빌드 26200은 넘침 영역의 아이콘에 자리가 없다고 하지 않고 넘침 단추의
  자리를 주므로, 거기서는 보이지 않는 채로 움직입니다. 쉬고 있을 때는 늘 그랬던 그 아이콘 그대로이고, 배지와
  모양도 그대로입니다.
- 설정 창이 열려 있는 동안에는 작업 표시줄 단추도 같은 리듬, 같은 프레임으로 똑같이 움직입니다.
  단추의 상태는 같은 워처에 대한 아이콘의 상태이고, 팝업이 열려 있을 때 아이콘이 갖는 상태입니다.
  창도 팝업처럼 워처를 지금 그대로 읽기 때문입니다. 같은 설정에서, 그리고 창이 숨겨져 있을 때
  멈춥니다. 제목 표시줄의 아이콘은 움직이지 않고, 쉬고 있을 때 단추는 늘 그랬던 그 아이콘입니다.
...
TO FILL: 무엇을, 어떤 빌드로, 어떤 기계에서(Windows 빌드, 화면, 배율, 언어, 앱 모드) 실행했는지.

- **테스트.** TO FILL
- **창을 두 번 빌드.** TO FILL
- **잘림.** TO FILL
- **v0.6.4 위로 설치.** TO FILL
- **실제로 본 호환성 카드와 새로 받기.** TO FILL
- **업데이트 확인과 그에 딸린 새로 받기.** TO FILL
- **실제 바탕 화면의 알림 카드.** TO FILL
- **카드가 나타나면 안 될 때의 Windows 자체 알림.** TO FILL
- **아이콘의 움직임.** TO FILL
- **상태 불빛.** TO FILL
- **드롭다운 목록, 스위치, 화면 읽기 프로그램.** TO FILL
- **한국어와 라틴 문자.** TO FILL

확인하지 않았고 주장하지도 않는 것: TO FILL
