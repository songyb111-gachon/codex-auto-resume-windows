...
## v0.6.4 — 밝게도 어둡게도 하나의 모습, 차분해진 상태 불빛, 곧바로 열리는 창
...
- 대시보드와 알림 영역 팝업이 Codex 안의 패널과 같은 재질을 씁니다. 떠 있는 카드와 안으로 들어간
  ...
  (아래에 적은 스위치 자리를 빼면) 그대로이고, 알림 영역 아이콘과 그 배지도 그대로입니다.
...
- 팝업에서는 작업마다의 스위치가 행의 오른쪽 끝, 이름표 첫 줄과 나란한 자리로 옮겨졌고, 이름표는
  그 왼쪽에 남은 자리에서 줄을 바꿉니다.
...
- 가장 긴 설정 부분에 맞춰 커지는 대신 1000 × 600으로 열립니다. 아홉 개 언어 모두에서 100%부터 200%까지의
  배율로 개요가 스크롤 없이 다 들어가는 너비이고, 150% 배율의 1920 × 1080 화면에도 들어갑니다. 창보다 긴
  설정 부분은 스크롤됩니다.
- 개요의 카드마다 있는 단추가 내용 아래가 아니라 제목 옆에 놓입니다. 대기 중 페이지의 **기다리는 이유**는
  옆의 목록만큼 높고 자기 카드 안에서 스크롤되므로, 페이지가 스크롤되지 않습니다.
...
- 빨라졌습니다. 개발 중에 배율 150%, 빈 설치에서 측정해 보니 머리글이 처음 그려질 때까지 1.46–1.54초에서 0.44–0.45초로, 이후 페이지 전환은
  중앙값 약 100ms(최대 203ms)에서 22ms(최대 59ms)로, 이어서 하기 메시지 부분은 222–241ms에서 74–82ms로,
  브리지의 상태 확인은 341ms에서 168ms로 줄었습니다.
