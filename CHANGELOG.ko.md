...
## v0.6.6 — 은은해진 상태등과, 설치된 곳에서도 움직이는 작업 표시줄 단추

[이 릴리스의 커밋](https://github.com/songyb111-gachon/codex-auto-resume-windows/compare/v0.6.5...v0.6.6)

하루 써 보고 드러난 두 가지입니다. 복구에 대한 것은 아무것도 바뀌지 않았습니다. 같은 분류기, 같은
관문, 보낼 수 있는 것은 여전히 워처 하나뿐이고, 새로 생긴 설정도 없습니다.

### 상태등이 부드러워졌습니다

- v0.6.5의 불빛은 숨쉬기보다 깜빡임으로 읽혔습니다. 한 주기가 3.2초에서 4.4초로 길어졌고, 점은
  자기가 놓인 카드 쪽으로 60%가 아니라 38%만 어두워지며, 점이 다 밝아진 뒤 번지는 빛의 세기도
  0.34에서 0.30으로 내려왔습니다. 모양은 v0.6.5 그대로입니다. 같은 네 단계, 같은 falloff, 같은
  3px입니다. 복구를 보내는 동안에는 같은 주기가 2초가 아니라 2.6초마다 돌고, 사람이 필요한 상태는
  여전히 1.4초에 한 번만 돌고 켜진 채 멈춥니다.
- 표 하나를 모든 표면이 함께 읽으므로 창과 알림 영역 팝업, Codex 안의 패널, 알림 카드가 같이
  부드러워지고 여전히 같은 불빛입니다.
- 알림 영역 아이콘과 창의 작업 표시줄 단추도 같은 리듬을 읽습니다. 그래서 마크의 머리도 더 천천히
  숨쉬고, 고리를 따라 도는 회전은 16초가 아니라 22초마다 옵니다. 머리 자체는 여전히 배지 쪽으로
  60% 어두워집니다. 열여섯 픽셀에서는 새로 정한 38%가 아예 읽히지 않기 때문입니다.
...
- **테스트.** 2,556개, 실패 없음. 여기에서 Python 3.13(759초, 8개 건너뜀), 그리고 GitHub의 Windows
...
- **창을 두 번 빌드.** 같은 소스에서 만든 `CodexAutoResumeSettings.exe` 두 개가 바이트까지 같았습니다
  (`7c95119e…`, 382,976바이트). 아카이브 자신의 digest는 여기에 적지 않습니다. 이 파일이 아카이브 안에
...
- **불빛.** 숫자는 창·팝업·패널·카드 네 표면에서 실제로 그려진 픽셀로 테스트가 붙잡고 있고, 문서의
  모든 그림은 제품 자신의 코드로 새 표에서 다시 그렸습니다. 아홉 개 언어, 밝게와 어둡게 모두입니다.
- **아이콘의 움직임.** 문서의 GIF는 새 리듬의 아이콘 프레임으로 만들었고, 테스트가 그 프레임 하나
  하나를 아이콘 자신의 표와 맞춥니다.
