...
수정 하나를 v0.6.0까지 기다리지 않고 따로 내보냅니다. 이전 모든 릴리스에 있는 코드 주입 문제이기
때문입니다. 그 밖에는 아무것도 바뀌지 않습니다. 복구 동작, 설정, 상태, 설치 구조는 v0.5.6과
똑같습니다. 업그레이드만 하면 해결됩니다.

### 보안

- **고침: 폴더나 대화 이름으로 PowerShell 명령을 실행할 수 있었습니다.** v0.5.6을 포함한 이전
  모든 릴리스에 해당합니다. 알림과 시작 메뉴 바로 가기는 짧은 Windows PowerShell 스크립트로
