...
- **지금** 카드는 워처, Codex 엔진, 마지막 확인을 먼저 보이고 자동 복구를 마지막에 보이며, 자동 복구는
  그것을 일시 정지하고 다시 켜는 단추와 같은 줄에 놓입니다. 이 카드는 보일 수 있는 가장 긴 낱말에 맞춰
  배치되므로 워처의 상태가 바뀌거나 마지막 확인이 오래되어도 움직이지 않고, 워처가 어떤 상태이든 개요는
  여전히 창 안에 다 들어갑니다.
