...
**사전 릴리스이며, 아무에게도 배포되지 않습니다.** `dev` 브랜치에서 GitHub 사전 릴리스로
게시되므로 `releases/latest`는 이것을 답하지 않고, 어떤 설치에도 제안되지 않습니다. 플러그인의 설치
경로는 `main`이 말하는 것을 설치하며, 그것은 여전히 v0.6.9입니다. 이 릴리스가 있는 이유는 v0.6.10
작업을 나눠야 하는 한 단계이고 그 자체로는 릴리스가 아니기 때문입니다. 사람이 볼 수 있는 것은 아무것도
바뀌지 않습니다. 창은 그리던 것을 그리고, 팝업은 같은 픽셀을 렌더링하고, 패널은 같은 스크립트를
...
- README는 짧은 안내가 되었습니다. 무엇을 하는지, 어떻게 설치하는지, 알아야 할 제한 하나, 나머지가 어디
  있는지입니다. 담고 있던 내용은 모두 [안내서](docs/GUIDE.ko.md)에 있습니다.
...
- 같은 규칙은 v0.5.3 시점에 이미 로그인 항목, 알림 식별자, 시작 메뉴 바로 가기, 알림 핸들러를
  덮고 있었습니다. 이제 [SECURITY.md](docs/SECURITY.md)가 모든 종류의 자원에 대해 그것을 한 번에
  서술합니다.
...
- **고침: 이 도구는 자기 디렉터리 바깥에도 상태를 남깁니다.** 로그인 값, 알림 발신자 식별자,
  ...
  설명하는 곳에 함께 적혀 있습니다.
- **고침: [SUPPORT.md](docs/SUPPORT.md)가 비공개 보안 창구를 가리키고 있었습니다.**
  [SECURITY.md](docs/SECURITY.md)는 그런 창구가 없다고 말하는데도 말입니다.
...
- **[PRIVACY.md](PRIVACY.md)** — 무엇을 읽고, 무엇을 저장하며, 어딘가로 무엇을 보내는지에 대한
  ...
  나가는 요청 없음.
- **[SUPPORT.md](docs/SUPPORT.md)** — 어떤 종류의 문제를 어디에 신고할지, 무엇을 담을지, 공개 이슈에
  무엇을 붙여넣지 말아야 할지.
- **[CONTRIBUTING.md](docs/CONTRIBUTING.md)** — 테스트를 실행하고 릴리스를 빌드하는 방법, 픽스처
