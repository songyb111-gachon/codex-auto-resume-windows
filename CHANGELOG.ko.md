...
실패를 알리는 불빛은 나타날 때 한 번 맥박친 뒤 멈춰 있었습니다. 이제 실패가 보이는 동안 계속 숨
쉽니다. 2.2초마다 한 주기로, 복구 중의 2.8초와 감시 중의 4.4초보다 조금 빠르고, 곡선과 번짐은 다른
불빛과 같습니다. 창, 알림 카드, 트레이 아이콘, 작업 표시줄 단추가 모두 한 표를 따릅니다. 확인이 필요할
때는 여전히 한 번 맥박친 뒤 머물고, 움직임 줄이기, Windows의 애니메이션 설정, 고대비에서는 여전히 모든
불빛이 멈춰 있습니다.

### 그 밖에

- README의 아이콘 움직임 그림에 다섯 번째 칸, 숨 쉬는 실패 아이콘이 생겼고 배지는 없어졌습니다.
- 로드맵: Python 모듈화는 프리 릴리스인 **v0.6.9-alpha**가 되고, 저장소 전체의 버그 찾기를 마친 최종
  **v0.6.9**가 마지막 Python 릴리스입니다. Rust 릴리스들은 한 칸씩 당겨져 v0.6.10 - v0.6.12가 됩니다.
