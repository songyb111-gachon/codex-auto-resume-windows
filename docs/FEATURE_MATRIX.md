...
- **이번 릴리스가 공개된 가장 새 릴리스입니다.** 아래에서 PUBLISHED라고 적힌 항목들이 뜻하는 것은
  공개된 릴리스, 곧 v0.6.0, v0.6.1, v0.6.2와 이번 릴리스입니다. 넷 다 압축 파일이 발행되었고 해시가
  `main`에 고정되어 있습니다. 이번 릴리스는 화면 언어 아홉 개, 이어가기 메시지 설정, 알림 영역 아이콘의
  팝업, 알림의 대시보드 열기 단추, 새로 꾸민 창과 패널을 더했고, 그 앞의 세 버전 모두에 있던 결함
  하나 - 복구되는데도 끌 스위치가 없던 중단 종류 - 를 고쳤습니다. 그 기능들에 대한 항목은 여전히
  무엇으로 시험했는지를 적고 있으며, 어느 것도 PUBLISHED라고 하지 않습니다. 발행된 바이트로 다시
  돌려 보지 않았기 때문입니다. 이 문서 어디에서도 PUBLISHED가 뜻하지
  *않는* 것은, 누군가 그것을 설치해서 써 보았다는 것입니다. 발행된 바이트는 그 확인만을 위해 만든
  ...
  일은 [LIVE_ACCEPTANCE.md](LIVE_ACCEPTANCE.md)가 맡습니다. 그 절차는 v0.6.0을 상대로
...
| 모든 공개 코드와 덧보기에 모든 언어의 표시 문구가 있다 | UNIT TESTED | `tests/test_labels_v3.py:LabelTests.test_every_public_code_and_overlay_has_a_label_in_every_language`; `tests/test_locale.py:CatalogTests`(9개) | 돌아가는 창에서 여덟 번역 가운데 어느 것이든 읽어 보았다는 기록이 없다. 커밋된 스크린샷에는 영어와 한국어만 있고, 그 그림들은 이번 릴리스보다 앞선다. |
...
| 머리글의 상태 점은 워처가 무엇을 하는지 말한다 - 감시 중, 기다리는 중, 때가 된 작업을 확인 중, 복구 중, 일시 중지됨, 확인이 필요함 - 일하는 동안에만 되풀이해 움직이고, 경보에는 한 번만 깜박이며, 움직임을 줄이면 전혀 움직이지 않는다 | REAL WINDOWS TESTED | `tests/test_gui_v063.py:AliveStateTests`(7개)가 내장 C# 컴파일러로 창을 컴파일한 뒤, 워처 상태와 기다리는 일의 조합 열한 가지와 여섯 시점에 대해, 움직임 줄이기를 끄고 켜며, 창 자신의 정적 판단·불투명도 메서드를 리플렉션으로 부른다 | 내장 컴파일러나 PowerShell이 없으면 건너뛴다. 메서드를 부를 뿐 창을 띄우지는 않고, 움직이는 머리글을 찍은 캡처도 없다. 움직임 줄이기는 인자로 바로 넘기므로, 이 테스트들이 바꾸어 보는 것은 Windows 자체의 애니메이션 설정이 아니다. |
...
| 대기 중의 자동 이어 가기 확인란은 한 작업의 정확한 중단과 대화에 묶여 있고, 그사이 끝났거나 사라졌거나 다른 대화의 것으로 드러난 레코드에 닿은 클릭은 거절되어 아무것도 바꾸지 않으며, 켜도 아무것도 보내지 않고 일정도 옮기지 않는다 | UNIT TESTED, 제어 계층에서 | `tests/test_control_continuation.py:InterruptionSwitchTests`(5개, `test_a_click_for_another_conversation_changes_nothing`, `test_a_record_that_is_gone_or_finished_is_refused`, `test_switching_on_sends_nothing_and_changes_no_schedule` 포함); `BridgeCommandTests.test_the_window_reaches_all_three` | 확인란이 자기가 그려진 행의 id를 보낸다는 C# 쪽 절반은 구동하지 않는다. 같은 스위치의 팝업 쪽은 진짜 창에서 구동한다(8절). |
...
| 보여 주는 것: 가장 급한 작업부터 많아야 셋, 저마다 재설정이나 다음 시도까지의 카운트다운, 대화에서 가져오는 것은 대화 이름뿐, 꺼 둔 대화는 체크가 빠진 채, 그려질 때의 상태에 따른 일시 정지나 다시 켜기, 그리고 카탈로그에서 온 모든 문구 | UNIT TESTED | `tests/test_tray_popup.py:ViewModelTests`(11개), `ActivityTests`(3개) | 테스트 안에 적은 레코드로 만든다. |
...
| 배치: 모든 배율과 모든 언어에서 누를 곳이 모두 카드 안에 있고, 줄바꿈한 글에는 잰 만큼의 높이를 주고, Tab과 Shift+Tab으로 돌며, 문자 체계마다 글꼴이 있다 | UNIT TESTED | `tests/test_tray_popup.py:LayoutTests`(8개), `FontTests`(4개) | 글자는 진짜 렌더러가 아니라 글자마다 폭이 고정된 대역으로 잰다. |
...
| 진짜 창을 만들고 그리고 없애도 새는 것이 없고, 여러 번 열고 닫아도 붙드는 것이 없으며, 대기 표시 아이콘을 만들고 놓아 주고, 한국어는 낱말 사이에서 줄을 바꾸며, 언어가 바뀌면 글꼴을 다시 만든다 | REAL WINDOWS TESTED | `tests/test_tray_popup.py:WindowsTests`(12개)가 화면 밖에 창을 만들고, 쉰 번을 돌리기 전과 뒤의 프로세스 GDI·USER 자원 수를 견준다. `BadgeTests`(2개) | Windows가 아니면 건너뛴다. 창은 화면 밖에 띄울 뿐 아무도 보지 않는다. 팝업을 찍은 커밋된 그림도, 진짜 아이콘을 사람이 클릭한 기록도 없다. |
| 움직임은 머리글과 같은 규칙을 따르고, 움직임 줄이기는 Windows 자체 설정보다 앞서 모든 되풀이와 깜박임을 멈춘다 | UNIT TESTED | `tests/test_tray_popup.py:MotionTests`(7개, `test_reduced_motion_never_loops_or_pulses` 포함), `ReduceMotionSettingTests`(2개) | 화면 위의 움직임을 재는 것은 없다. |
...
| 도구 열일곱 개 - 이번 릴리스에 읽기 전용 `preview_recovery_message`가 더해졌다 - 가 각각 스키마와 구현을 갖고, 스킬 문서가 정확히 그 목록을 적는다 | UNIT TESTED | `tests/test_mcp.py:ToolSurfaceTests.test_every_tool_has_a_schema_and_an_implementation`, `test_the_skill_lists_every_tool_and_no_others` | - |
...
| 새로 꾸민 패널: 움직임 줄이기가 모든 애니메이션과 전환을 멈추고, 쪽에서 재깍이는 것이 없고, 초점 테두리는 키보드에만 그리며, 색 토큰은 어떤 테마 블록보다 먼저 모두 선언된다 | UNIT TESTED | `tests/test_mcpui_v063.py:StyleTests`(8개) | 내주는 스타일시트에 대한 확인이다. 커밋된 패널 그림은 새로 꾸미기 전의 것이다. |
...
| `codex-auto-resume:` 프로토콜 처리기는 사용자별로 등록되고, activate 동사만 싣고, 우리 것일 때만 지워진다 | UNIT TESTED | `tests/test_notify.py:ProtocolRegistrationTests`(4개); `tests/test_cli.py:DoctorProtocolCheckTests` | 가짜 레지스트리를 상대로 한다. **실제 프로토콜 등록도, 실제 알림 버튼에서 온 활성화 기록도 없다.** 이번 릴리스부터 같은 동사가 대시보드 열기 요청도 싣는데, 그것을 덮는 테스트는 없다(11절). |
...
| 공개된 버전의 자산은 덮어쓸 수 없고, 수동 실행으로는 게시할 수 없다 | UNIT TESTED | `tests/test_convergence.py:ReleaseImmutabilityTests`(5개) | 릴리스 자체는 GitHub의 불변 릴리스가 아니다. 이 규칙은 워크플로를 묶을 뿐 쓰기 권한을 가진 모두를 묶지 않는다. [VERIFY.md](VERIFY.md)가 그렇게 적고 있다. |
...
| 알림의 대시보드 열기 | IMPLEMENTED | 여는 URI를 만들고 읽고 활성화하는 테스트, 그리고 [LIVE_ACCEPTANCE.md](LIVE_ACCEPTANCE.md) 4단계에서 실제 알림을 클릭하고 남긴 메모. |
| 탐색기가 다시 시작된 뒤 돌아오는 아이콘 | IMPLEMENTED | 워처가 도는 동안 탐색기를 한 번 다시 시작하고, 아이콘이 돌아왔다고 메모하기. |
| 실제 알림 영역 아이콘에서 여는 팝업 | REAL WINDOWS TESTED, 화면 밖에서 그린 그림으로(`docs/images/tray-popup.png`와 ko, ja, zh-CN, de 짝) | 아이콘에서 열고, Esc와 바깥 클릭으로 닫고, 실제로 기다리는 작업에서 체크박스를 쓰는 과정을 기록한 실제 점검. |
| 라이브 인수 절차 | UNVERIFIED | 누군가 실제 기계에서 [LIVE_ACCEPTANCE.md](LIVE_ACCEPTANCE.md)의 절차를 돌리고, 그 결과 파일을 `docs/evidence/live/` 아래에 커밋하기. |
...
가장 얇은 쪽은 표면입니다. 창의 여섯 쪽은 진짜 코드지만 캡처는 셋뿐이고 모두 이번 릴리스의 새
모양보다 앞서며, 알림 영역 아이콘과 새 팝업은 몇 개의 테스트에서 화면 밖에 진짜로 만들어졌다
사라질 뿐 어느 쪽도 그림 한 장 없고, Codex 패널은 Codex가 한 번도 불러온 적이 없습니다. 이번
릴리스의 요청 함수가 Node에서 돌 뿐입니다. 표면이 제어 계층과 이야기하는 곳에서는, 제어 계층은 잘 시험되어
