...
- **인수할 릴리스 후보.** 인수하려는 버전으로 빌드된 압축 파일과 그 옆에 게시된 `.sha256`.
  ...
  게시된 v0.6.2입니다.
- **이번 릴리스에서 더해진 것은 단계 안에서 확인합니다.** 아래의 **이번 릴리스에서 더해진 확인** 절이
...
영역에서 아이콘을 찾아 툴팁을 보고, 오른쪽 클릭으로 메뉴를 엽니다. 이번 릴리스부터 왼쪽 한 번 클릭은 메뉴
대신 작은 창을 여는데, 그 창은 아래의 **이번 릴리스에서 더해진 확인** 절에서 봅니다. 그 메뉴에서 워처를 멈추고 아이콘이 사라지는 것을
본 뒤, 창에서 다시 시작합니다.
...
이번 릴리스라면 게시된 v0.6.2 설치본에서 시작해 그 위에 이번 압축 파일을 설치합니다. 이 업그레이드는
기본값 하나를 일부러 바꿉니다. 연속 메시지는 중단 종류마다 고정된 영어 문장 하나였다가, 이제 이어서 하기
언어로 쓴 기본 메시지가 됩니다. 그러니 뒤에 설정을 읽어 보세요. **화면 언어**는 시스템, **메시지
스타일**은 기본, 직접 입력 메시지는 설정하지 않음이어야 하고, 그 밖의 것은 메모에 적습니다.
...
## 이번 릴리스에서 더해진 확인, 그리고 적을 곳

이번 릴리스는 제품이 무엇을 말하고 보여 주는지를 바꿀 뿐, 복구가 무엇을 결정하는지는 바꾸지 않습니다.
