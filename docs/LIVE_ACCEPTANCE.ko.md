...
이번 릴리스라면 게시된 v0.6.3 설치본에서 시작해 그 위에 이번 압축 파일을 설치합니다. 이번 릴리스는
설정을 하나도 바꾸지 않으므로, 뒤에 설정을 읽어 보세요. **화면 언어**, **메시지 스타일**, 직접 입력
메시지는 업그레이드 전과 같아야 하고, 그 밖의 것은 메모에 적습니다. v0.6.2나 그 이전에서 시작하면
