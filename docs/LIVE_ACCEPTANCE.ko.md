...
| 대기 중의 자동 이어 가기 확인란 | 10단계 | `retry-now` |
| 모두 취소 | 12단계 뒤 | `upgrade-keeps-decisions` |

**화면 언어 — 3단계, `watcher-starts`에 메모.** 환경에 `CODEX_AUTO_RESUME_LANG`이 있으면 설정보다
앞서므로 먼저 지우세요. **설정 → 일반**에서 **화면 언어**를 Windows가 쓰지 않는 언어로 바꿉니다. 창은
이미 열려 있는 것은 다음에 열 때 바뀐다고 말합니다. 대시보드를 닫았다 다시 열고, 팝업을 열고, 아이콘
위에 포인터를 올립니다. 그다음 메뉴에서 워처를 멈추고 창에서 다시 시작한 뒤, 로그아웃했다가 다시
로그인합니다. 끝나면 **시스템**으로 되돌립니다. *통과:* 바꾼 뒤에 연 것은 모두 고른 언어로 말하고,
...
팝업에서 그 작업의 확인란을 클릭합니다. 대기 중 쪽 자신의 확인란은 새로 고침 사이의 5초 안에 작업이
...
**대기 중의 자동 이어 가기 확인란 — 10단계, `retry-now`에 메모.** 기록이 기다리는 동안 대기 중 쪽에서 그
작업의 **자동 이어 가기** 확인란을 끄고, 다시 켜고, 그때마다 팝업에서 같은 작업을 봅니다. *통과:*
확인란이 꺼져 있는 동안 그 작업에 대해 아무것도 보내지지 않습니다. 다시 켜도 클릭 때문에 보내지는
