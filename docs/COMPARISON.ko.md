...
| 상주 프로세스 | supervisor, worker, 선택적 app-server, MCP 서버 | watcher 하나, 그리고 Codex가 도는 동안의 MCP 서버 |
| 인터페이스 | tray 아이콘, 설정 창, Codex 패널 | 시작 메뉴 창, Codex 패널, 알림 |
...
| **tray 아이콘과 supervisor 프로세스** | 하나를 눈에 보이게 하려고 상주 프로세스를 둘 더 두는 셈입니다. 시작 메뉴 항목과 알림이 같은 필요를 채웁니다 |
...
| **대기 중인 복구별 실시간 카운트다운** | 표시만 바뀌는 기능이고, 실제로 쓸모도 있습니다. 그것이 중요한지 알 수 있을 만큼 패널이 쓰이기 전까지는 기다립니다 |
