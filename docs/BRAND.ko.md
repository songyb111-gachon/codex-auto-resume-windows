...
머리는 `GLOW`의 감시 리듬으로 배지의 짙은 파랑 쪽으로 흐려졌다 돌아오며 숨 쉬고, 이따금 한 번씩 자기 고리를
천천히 한 바퀴 돕니다. 복구가 진행되는 동안에는 `GLOW`의 호 리듬마다 한 바퀴씩 계속 돌며 복구 중의 숨을
쉽니다. 일시 정지되면 회색이고 멈춰 있습니다. 문제가 생기면 그 색이 되어 `GLOW`의 attention 리듬으로 한 번
맥박친 뒤 머뭅니다. 움직임은 모양도 색도 더하지 않습니다. 프레임은 `.ico`와 같은 래스터화기로 `ICON_SHAPE`에서
그린 마크 자체이고, 머리만 옮기고 다시 칠할 뿐이며, 쉬고 있는 아이콘은 늘 그랬던 그 아이콘 그대로입니다.
아이콘이 `GLOW`에서 읽는 것은 그 네 리듬뿐입니다. 아이콘 자신의 몇 가지 숫자 - 감시 중인 아이콘이 얼마나
자주 도는지, 머리의 자리 수, 숨의 밝기 단계 - 는 `tray.py`의 `ICON_MOTION`에 있고, 일부러 `brand.py`에
두지 않았습니다. `GLOW`의 키는 모두 창으로 생성되는데, 아이콘을 그리는 창은 없기 때문입니다. 움직임
줄이기, Windows의 애니메이션 설정, 고대비, 배터리 절약 모드에서, 세션이 잠겨 있을 때, 그리고 넘침 영역에
있는 동안에는 멈춰 있습니다.
