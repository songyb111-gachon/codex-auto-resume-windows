...
| 클래식 (v0.6.2) | `CLASSIC_LIGHT`, `CLASSIC_DARK` | 아니요 | 예 | 예 | 예 | 예 | 8, 7, 6, 4 |
| 단순하게 | `PLAIN_LIGHT`, `PLAIN_DARK` | 아니요 | 아니요 | 예, 흐려지기만 | 예 | 아니요 | 8, 4, 4, 4 |
...
- **컨트롤이 미끄러짐**은 스위치나 체크박스가 바뀔 때 움직이는 것, 목록이 떠오르며 열리는 것, 스크롤이
  미끄러지는 것, 알림 카드가 들어오고 나가고 미끄러지는 것입니다. 부드럽게, 움직임 없이를 뺀 모든 디자인이
  미끄러지며, 그 디자인에서만 컨트롤이 그냥 바뀝니다. 움직임을 거두는 디자인은 그것 하나입니다. 클래식은
  v0.6.2의 모양에 지금의 상태 불빛과 지금의 움직임이고, 단순하게도 부드럽게처럼 움직입니다.
