...
| 실패 | `danger` | 2.2초마다 한 주기. 복구 중보다 조금 빨라서, 빨간 불빛은 멈춰 있는 법이 없습니다 |
| 사람이 필요함 | `attention` | 처음 보일 때 1.4초에 걸쳐 한 주기, 그 뒤로는 빛 없이 켜진 채 멈춰 있습니다 |
...
28픽셀 칸 안에 넉넉히 들어갑니다. 이번 릴리스부터는 알림 카드의 불빛도 같은 표를 따라 숨 쉬며, 그
면은 많아야 80ms마다 한 번 다시 그려지고, 움직임 줄이기와 고대비 모드에서는 멈춰 있습니다. v0.6.6은
일부러 한 번만 그려 켜진 채 멈춰 두었습니다. 불빛 옆에는 늘 그 단어가 있습니다.
...
되어 `GLOW`의 attention 리듬으로 한 번 맥박친 뒤 머뭅니다. 실패하면 빨간색이 되어, 실패가 이어지는 동안
`GLOW`의 failed 리듬으로 제자리에서 계속 숨 쉽니다. 움직임은 모양도 색도 더하지 않습니다. 프레임은
...
읽는 것은 세 리듬뿐입니다. 한 주기의 칸과 복구 중 쓸고 오는 한 번의 길이가 되는 감시 숨, 실패의 숨, 그리고
attention 맥박입니다. 마크 위에는 아무것도 덧그리지 않으므로(모서리의 상태 배지가 없습니다) 알림 영역
