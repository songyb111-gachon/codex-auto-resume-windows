...
| 기한이 된 작업을 확인하는 중 | `active` | 빛 없이 켜져 있고, 가는 호가 1.6초에 한 바퀴 돕니다(대시보드와 팝업, 이번 릴리스부터는 패널에서도) |
...
중이라고 했습니다. 이제는 같은 규칙으로 쪽의 시계를 읽고 행에 적힌 시각이 오면 한 번 깨어나, 확인 중이라고
하고 그 행에 *지금*이라고 적습니다. 그 뒤에 워처가 무엇을 했는지는, 패널의 다른 모든 것처럼 다음 읽기와
함께 옵니다. 알림 영역 아이콘과 창의 작업 표시줄 단추는 자기 규칙을 따르며, 그 규칙에서는 아직 아무도 보지
