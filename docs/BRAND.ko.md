...
이번 릴리스는 셋을 비슷한 재질 세 가지가 아니라 하나의 재질로 만들었습니다. 기준은 Codex 안의
...
| 표면 | 팔레트를 가져오는 방법 |
| --- | --- |
| Codex 패널 | `mcpui.py`가 import 시점에 `brand.LIGHT`와 `brand.DARK`, `brand.css_scale()`, 그리고 그림자 레시피를 CSS로 써 주는 `brand.css_elevation()`으로 `:root` 블록을 만듭니다. |
| 대시보드 | `build/make_brand.py`가 `gui/Brand.cs`를 생성합니다. 밝은 테마의 모든 토큰은 `Color`로, 스케일과 레이아웃 크기와 그림자 레시피와 상태 불빛의 숫자는 상수로, 상태 불빛의 상태별 규칙은 생성된 작은 메서드로 들어갑니다. `gui/Controls.cs`는 색을 `Palette` 클래스 하나를 거쳐 읽고, 고대비 모드를 반영하는 곳도 그 한 곳입니다. 생성된 파일을 커밋해 두므로, Python이 없는 기여자도 창이 어떤 모습일지 읽어 볼 수 있습니다. |
| 알림 영역 팝업 | `tray_popup.py`가 `brand`를 import해 `brand.LIGHT`, 스케일, 그림자 레시피, `brand.glow()`로 직접 그립니다. |
...
| `LAYOUT` | button 높이 34, field 35, switch 40 × 22, chip 22, card padding 16 × 18, page gap 14 등 | 패널에서 가져온, 함께 쓰는 컨트롤 레시피의 크기. |
| `SHADOWS` | 밝은 테마의 card: offset (4, 4), blur 14, `shadow_dark` 0.55, 그리고 offset (−4, −4), blur 14, `shadow_light` 0.90; control: 같은 구성에 offset 2, blur 6; inset: offset 2, blur 6, 테두리 안쪽 | 떠오른 카드, 떠오른 컨트롤, 우물. 일부러 작습니다. 과장된 엠보싱이 부드러운 인터페이스를 읽을 수 없게 만듭니다. |
| `STATUS_DOT`, `STATUS_FILL`, `GLOW` | 점 반경: 창 5, 팝업 4.5, 패널 6; 빛은 점 바깥으로 7까지 | 상태 불빛: 그 크기, 상태마다의 색, 그리고 빛. |
| `MOTION` | transition 160 ms | 패널의 컨트롤 전환. |
...
`--space-*`, `--type-*`, `--lh-*`, `--size-*`, `--glow-*`, `--transition`을 받습니다.
...
상태 불빛은 대시보드 머리글, 팝업 맨 위, 패널 맨 위에서 워처가 살아 있는지를 말합니다. 평평한 점
둘레에 은은한 빛이 있고, 움직이는 것은 그 빛뿐입니다. `brand.glow()`가 그 빛을 한 번 정의하고, 세
표면 모두 그 하나의 함수에 비추어 테스트합니다.

| 상태 | 색 | 빛 |
| --- | --- | --- |
| 감시 중 | `active` | 천천히 숨 쉽니다. 3.6초에 걸쳐 불투명도가 0.14에서 0.30으로 오르고, 빛은 94%에서 제 크기까지 커집니다 |
| 대기 중 | `active` | 0.20에 멈춰 있습니다 |
| 기한이 된 작업을 확인하는 중 | `active` | 0.20에 멈춰 있고, 가는 호가 1.6초에 한 바퀴 돕니다(대시보드와 팝업) |
| 복구 중 | `active` | 조금 더 빠르게 숨 쉽니다. 2.2초에 걸쳐 0.18에서 0.38로 |
| 사람이 필요함 | `attention` | 1.4초에 걸쳐 0.20에서 0.42까지 한 번 은은하게 맥동했다가 돌아오고, 그 뒤로는 멈춰 있습니다 |
| 일시 정지, 멈춤 | `paused`, `idle` | 없음 |

빛은 점 바깥 7픽셀에 걸쳐 흐려지고, 어디에도 경계가 없습니다. 깜빡이는 것은 없고, 불빛 옆에는 늘 그
단어가 있습니다.
...
그립니다. 그 교체가 일어나는 곳이 `gui/Controls.cs`의 `Palette`입니다. 이번 릴리스부터는 팝업도 같은
대응으로 같은 교체를 하고, 패널에는 강제 색상(forced-colors) 스타일이 생겨 불빛, 스위치 손잡이,
드롭다운 화살표가 보이는 채로 남습니다. 세 표면 모두에서 상태 불빛은 빛 없이 시스템 색의 단색 점이
됩니다.
...
고칠 수 없습니다. v0.6.3의 부드러운 컨트롤은 표준 컨트롤을 대체하지 않고 그 위에 칠하므로, 그
...
- **밑에는 표준 컨트롤이 있다.** 대시보드에서 버튼은 여전히 `Button`이고, 스위치는 스위치 모양으로
  그린 `CheckBox`이며, 드롭다운은 닫힌 모습을 우리가 그리는 `ComboBox`입니다. 칠하는 것만 우리
  몫이므로, 키보드, 포커스, 화면 낭독기는 늘 그랬던 대로 동작합니다.
