...
이번 릴리스는 그 모습을 매듭짓고, 그것을 네 가지로 그립니다. 점검은 세 머리글을 패널의 것 하나의
방식으로 만들었고, 불빛의 규칙 하나, 버튼과 칩 하나, 콜아웃 하나, 한 가지에 한 단어를 정했습니다.
그리고 **디자인** 설정은 제품 전체를 부드럽게(위의 모든 것), 부드럽게, 움직임 없이, 클래식(v0.6.2의
...
| `LAYOUT` | button 높이 34, field 35, switch 40 × 22, 이름표까지 10을 띄운 체크박스 18, chip 22, 줄이 시작하는 곳에서 9, 단어에서 14 떨어진 상태 불빛, 10 × 12로 여백을 두고 18짜리 배지를 단 콜아웃, card padding 16 × 18, page gap 14 등 | 패널에서 가져온, 함께 쓰는 컨트롤 레시피의 크기. |
...
이번 릴리스부터 버튼과 칩은 모든 표면에서 같습니다. `LAYOUT`의 34픽셀 버튼과 양옆에 9씩 여백을 둔
22픽셀 칩이고, 그 글자는 `TYPE_ROLES`의 굵기 500이며 Windows는 이를 보통 굵기로 그립니다. 그 전에는
팝업과 알림 카드의 버튼이 높이 32에 굵은 글자였고, 칩도 굵은 글자에 더 빠듯해서, 창과 패널의 보통
굵기 옆에 섰습니다. 따로 짚어 둔 알림은 모든 표면에서 콜아웃입니다. `accent_soft` 바탕에 컨트롤의
...
- **상태가 앞선다.** 대시보드, 팝업, 패널에서 워처가 무엇을 하고 있는지가 가장 먼저 나오고 가장
  ...
  창을 여는 단 하나의 이유를, 보러 온 것이 아닌 모든 것 아래에 둔 셈입니다. 이번 릴리스부터 세
  머리글은 패널의 것처럼 짜입니다. 제품 이름이 있으면 흐린 머리말로 두고, 그다음 불빛과 상태의 단어를
  잉크색으로 두며, 불빛은 단어의 줄 위에 섭니다. 예전에는 팝업이 제품 이름을 제목으로 달고 그 아래 작은
  색 글줄로 상태를 말했고, 창의 불빛은 두 줄 사이에 서 있었습니다. 옆에 글이 있는 불빛은 모두 - 그 셋과
  알림 카드의 불빛 - 줄이 시작하는 곳에서 `LAYOUT`의 `light_inset`, 글에서 `light_gap`만큼 떨어져
  섭니다. 예전에는 그 간격이 15에서 18.5픽셀까지 제각각이었습니다.
- **단어 없는 색은 없다.** 상태는 자기 색을 옅게 깐 칩 위의 단어이고, 점 옆에는 늘 그 단어가
  있습니다. `active`는 텍스트를 싣지 않으며, 이번 릴리스부터 머리글의 단어는 상태의 색을 아예 싣지
  않습니다. 색은 불빛이 싣고, 뜻은 잉크색 단어가 싣습니다. 결과 하나는 어디에 보이든 한 색입니다. 기록의
...
릴리스부터(F15) 표면마다 자기 불빛을 알립니다. 불빛마다 위치, 반지름, 상태, 그리고 그 아래의 바탕이며,
