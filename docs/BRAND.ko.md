...
알림 카드가 무엇에 관한 것인지도 말합니다. 숨 쉬는 평평한 점이고, 번짐이 그 밝기를 따라갑니다.
...
움직이지 않는 회색 `idle` 불빛 옆에서 확인을 요청합니다. 실행 중이지만 상태가 좋지 않은 워처, 곧 이전
...
한 번 그려지고 시계가 없으므로, 다른 두 곳이 확인 중이라고 할 때 대기 중이라고 하고 그 행에 *지금*이라고
적습니다. 알림 영역 아이콘과 창의 작업 표시줄 단추는 자기 규칙을 따르며, 그 규칙에서는 아직 아무도 보지
...
28픽셀 칸 안에 넉넉히 들어갑니다. v0.6.7부터는 알림 카드의 불빛도 같은 표를 따라 숨 쉬며, 그
