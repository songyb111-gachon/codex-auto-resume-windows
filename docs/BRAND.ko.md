...
v0.6.4 다음 릴리스는 재질을 움직이게 하고 팝업에 입체감을 주었습니다. 상태 불빛은 눈에 보일 만큼 크게
숨 쉬고, 알림 영역 아이콘은 자기만의 더 작은 언어로 움직이며, 스위치는 미끄러지고 체크박스는 곡선
...
| `STATUS_DOT`, `STATUS_FILL`, `GLOW` | 점 반경: 창 5, 팝업 4.5, 패널 6; 빛은 점 바깥으로 7까지 | 상태 불빛: 그 크기, 상태마다의 색, 그리고 빛. |
...
릴리스부터는 알림 카드가 무엇에 관한 것인지도 말합니다. 평평한 점 둘레에 은은한 빛이 있고, 움직이는
것은 그 빛뿐입니다. `brand.glow()`가 그 빛을 한 번 정의하고, 세 표면 모두 그 하나의 함수에 비추어
테스트합니다.

| 상태 | 색 | 빛 |
| --- | --- | --- |
| 감시 중 | `active` | 숨 쉽니다. 3.2초에 걸쳐 불투명도가 0.12에서 0.58로 오르고, 밝아지면서 빛은 제 크기의 82%에서 108%까지 커집니다 |
| 대기 중 | `active` | 0.30에 멈춰 있습니다 |
| 기한이 된 작업을 확인하는 중 | `active` | 0.30에 멈춰 있고, 가는 호가 1.6초에 한 바퀴 돕니다(대시보드와 팝업) |
| 복구 중 | `active` | 더 빠르고 밝게 숨 쉽니다. 2초에 걸쳐 0.20에서 0.70으로, 88%에서 108%까지 커지며 |
| 사람이 필요함, 실패 | `attention`, `danger` | 1.4초에 걸쳐 0.30에서 0.72까지 한 번 맥동했다가 돌아오고, 그 뒤로는 0.30에 멈춰 있습니다 |
| 일시 정지, 멈춤 | `paused`, `idle` | 없음 |

빛은 점 바깥 7픽셀까지 닿고, 원판이 아니라 흐려지는 빛입니다. 닿는 거리의 대부분에서 절반쯤의
세기를 유지하다가(0.45 지점에서 0.58, 0.78 지점에서 0.50) 마지막 5분의 1에서 사라지므로, 가장 밝을
때는 점을 둘러싼, 부드럽지만 분명한 가장자리가 있는 고리로 읽힙니다. 한 번 어두워졌다가 다시 밝아지는
일은 없습니다. 점과 고리 사이의 틈은 과녁으로, 곧 경보의 모양으로 읽히기 때문입니다. 숨은 양 끝에 모서리가
없는 올린 코사인입니다. v0.6.4의 숫자(3.6초에 걸쳐 0.14에서 0.30, 닿는 거리의 10분의 7에서 이미
5분의 1로 떨어지는 빛)는 보통 거리에서 보이지 않을 만큼 조용했습니다. 지금의 묶음은 다섯 묶음을 표면마다
자기 모형으로 두 테마, 100%와 150%에서 그려 보고 고른 것입니다. 가장 큰 빛은 창의 점 중심에서
12.96픽셀까지 닿고, 이는 창이 모든 배율에서 그 빛에 내어 주는 28픽셀 칸 안입니다. 깜빡이는 것은 없고,
불빛 옆에는 늘 그 단어가 있습니다.

요청하면 모두 멈춥니다. 대시보드와 팝업은 **움직임 줄이기**(설정 > 모양)가 켜져 있거나 Windows
자체의 애니메이션 효과 스위치가 꺼져 있으면 모든 애니메이션을 멈추고, 대시보드는 고대비 모드에서도
멈춥니다. 패널은 대신 호스트의 `prefers-reduced-motion`을 따르므로, 패널에는 이 설정이 없습니다. 알림
