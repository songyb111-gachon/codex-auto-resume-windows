...
| 부분 | 들어 있는 것 |
| --- | --- |
| 일반 | 화면 언어, Windows 로그인 시 시작, Codex가 시작될 때 함께 시작, 알림 영역 아이콘, 표시할 알림 |
...
**Codex가 시작될 때 함께 시작**(v0.6.8 다음 릴리스)은 로그인 시 시작 옆에 있고, 켜기 전까지 꺼져 있습니다.
Codex는 열릴 때마다 이 플러그인의 MCP 서버를 시작하는데, 이 설정을 켜 두면 그 서버가 워처가 돌고 있지 않을 때
워처를 시작합니다. 로그인 때와 같은 실행기를 거치고, 설치가 진행 중일 때는 하지 않으며, 두 번째 워처는 띄우지
않습니다. 이를 위해 Windows에 새로 등록하는 것은 없고, 무엇을 복구하는지도 바뀌지 않습니다. Codex가 서버를
시작할 때마다 `logs\codex-start.log`에 무엇을 결정했는지 한 줄이 남습니다.

스위치는 동작하는 무언가를 켜고 끕니다(알림, 알림 영역 아이콘, 움직임 줄이기, 로그인 시 시작, Codex가 시작될
때 함께 시작, 대화 하나의 자동 복구). 체크박스는 목록에서 어떤 항목을 적용할지 고릅니다(위의 중단 종류, 표시할 알림).
