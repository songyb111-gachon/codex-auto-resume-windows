...
말하고 배지는 붙지 않으므로, 설정 창의 작업 표시줄 단추와 같은 그림입니다. 툴팁은 자동 복구가 일시 정지 상태인지, 몇 건이 대기 중이고 몇 건이
...
동안, 그리고 Windows가 아이콘을 아무도 보지 않는 넘침 영역에 두는 동안에는 움직이지 않습니다. 설정 창이
...
<img src="images/tray-popup-ko.png" alt="알림 영역 팝업. 대기 중 상태, 약 1분 30초 뒤 다음 확인, 대기 중인 작업 세 개가 각각 중단 종류, 다음 시도 시각, 자동으로 이어 가거나 다시 시도할지 정하는 스위치와 함께 보이고, 자동 복구 일시 정지와 대시보드 열기 버튼" width="360">
...
어느 알림이든 설정 창에서, Codex에서, 또는 `update_settings`로 끌 수 있습니다. 끄더라도 작업이
복구되는지는 달라지지 않습니다.
