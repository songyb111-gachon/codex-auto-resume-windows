# Codex Auto Resume 로드맵: v0.6.5부터 v0.6.11까지
...
**개발 중입니다. `dev` 브랜치에서 만들고 있습니다.**

v0.6.5에는 Codex 호환성 레지스트리와 v0.6.4를 써 보며 나온 화면 작업이 들어가고, Python 구현을
나누기 위한 기반을 마련합니다. 실제로 나누는 작업은 v0.6.6으로 옮겼습니다.
...
v0.6.6은 Python 구현의 구조를 크게 정리하는 릴리스로 계획하고 있습니다. 앞선 계획에서는 v0.6.5의
...
v0.6.7에서는 더 약한 가정이 필요하거나, 근거가 부족하거나, Codex가 지원하지 않는 동작에 기대야 해서
...
## v0.6.8 — Python 최종 감사와 안정화

v0.6.8은 Python 구현을 마지막으로 종합 감사하는 릴리스로 계획하고 있습니다.

v0.6.6과 달리 이 릴리스는 일부러 넓게 버그를 찾습니다.
...
## v0.6.9 — Rust 코어로 완전히 교체

v0.6.9에서는 제품에 들어가는 Python 코어를 Rust로 바꿀 계획입니다.
...
Rust 구현은 v0.6.8의 동작을 현실적으로 가능한 한 그대로 재현해야 합니다.
...
## v0.6.10 — Rust다운 구조 재편과 최적화

v0.6.9는 동작을 똑같이 맞추는 것을 먼저 챙깁니다.

그래서 첫 Rust 구현에는 Python 모양 그대로인 구조가 어느 정도 남을 수 있습니다.

v0.6.10에서는 코드베이스를 더 자연스럽게 Rust다운 모습으로 바꿀 계획입니다.
...
## v0.6.11 — Rust 최종 감사와 안정화

v0.6.11은 마지막 종합 안정화 작업으로 계획하고 있습니다.
...
v0.6.11 이후에는 주로 유지 보수 단계로 넘어갈 것으로 봅니다.
