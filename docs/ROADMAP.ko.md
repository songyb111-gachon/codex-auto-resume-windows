...
나누기 위한 기반을 마련합니다. 실제로 나누는 작업은 v0.6.8로 옮겼습니다.
...
v0.6.8은 Python 구현의 구조를 크게 정리하는 릴리스로 계획하고 있습니다. 앞선 계획에서는 v0.6.5의
...
GitHub에서 저장소의 첫 화면도 같은 릴리스에서 정리합니다. 어차피 경로가 옮겨지는 때에, 최상위에 놓인
파일을 줄입니다.
...
v0.6.9에서는 더 약한 가정이 필요하거나, 근거가 부족하거나, Codex가 지원하지 않는 동작에 기대야 해서
...
v0.6.10은 Python 구현을 마지막으로 종합 감사하는 릴리스로 계획하고 있습니다.

v0.6.8과 달리 이 릴리스는 일부러 넓게 버그를 찾습니다.
...
## v0.6.11 — Rust 코어로 완전히 교체

v0.6.11에서는 제품에 들어가는 Python 코어를 Rust로 바꿀 계획입니다.
...
Rust 구현은 v0.6.10의 동작을 현실적으로 가능한 한 그대로 재현해야 합니다.
...
v0.6.11은 동작을 똑같이 맞추는 것을 먼저 챙깁니다.

그래서 첫 Rust 구현에는 Python 모양 그대로인 구조가 어느 정도 남을 수 있습니다.

v0.6.12에서는 코드베이스를 더 자연스럽게 Rust다운 모습으로 바꿀 계획입니다.
...
이 문서는 지금의 방향을 기록합니다. v0.6.6은 공개되었고, 이번 릴리스는 `dev` 브랜치에서 만듭니다.
