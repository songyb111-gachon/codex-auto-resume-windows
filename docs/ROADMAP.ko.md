# Codex Auto Resume 로드맵: v0.6.7부터 v0.6.13까지
...
기본 복구 동작은 여기의 모든 릴리스 내내 늘 그랬듯 보수적으로 유지합니다. v0.6.10이 더하는 것은 사람이 켜는
것이고, 스스로 켜지는 일은 없습니다.
...
아이콘은 확인이 필요하다고 알립니다. 그때 계획한 고급 단계는 VERIFIED에만 열렸고, 아래 v0.6.10의 계획이 새로 더하는 것에 대해서는 그것을 풉니다.
...
얼마나 빠른지를 바꾸고, 그것만으로도 릴리스로 낼 만하므로 프리 릴리스 없이 냅니다.

### Codex와 함께 시작하기

지금은 Windows에 로그인할 때 워처가 시작됩니다. v0.6.9는 사용자가 요청한 다른 선택지를 더합니다. Codex가
시작될 때 함께 시작하는 것입니다. Codex는 열릴 때마다 이 플러그인의 MCP 서버를 이미 시작하므로, 그 서버가
워처가 돌고 있지 않을 때 워처를 시작할 수 있습니다. 로그인 때와 같은 실행기를 거치고, 설치나 업데이트가 진행 중일
때는 하지 않으며, 두 번째 사본은 절대 띄우지 않습니다. 이를 위해 Windows에 새로 등록하는 것은 없습니다. 예약 작업도,
WMI 구독도, Codex를 기다리며 상주하는 프로세스도 없습니다. 대시보드에서 켜기 전까지는 꺼져 있습니다.

v0.6.9에 넣을 수 있는지는 두 가지 사실이 정하고, 둘 다 만들기 전에 잽니다. Codex가 실제로 MCP 서버를 언제
시작하는지, 그리고 그렇게 시작된 워처가 ChatGPT 앱이 닫힐 때 어떻게 되는지입니다. 어느 하나라도 이 제품이
지키는 기준을 지킬 수 없다는 뜻이라면, 기준에 맞춰 억지로 구부리지 않고 v0.6.10의 고급판으로 옮깁니다.
...
## v0.6.10 — Python 모듈화, 그리고 두 판의 고급 기능

v0.6.10은 두 단계로 나옵니다. 프리 릴리스인 **v0.6.10-alpha**는 모듈화뿐이고, 최종 **v0.6.10**은 알파가
남긴 경계 위에 고급 기능을 올려 두 판으로 함께 냅니다. 이 순서는 일부러입니다. 한 판이 압축 파일에서 고급
코드를 뺄 수 있으려면 그 코드가 자기 모듈에 따로 있어야 하고, 나누기는 판이 둘일 때보다 하나일 때 하는 편이
안전합니다. 프리 릴리스는 정식 버전으로 내기에는 애매하지만 개발을 단계로 나누어야 할 때 씁니다. 모듈화가 그렇습니다.
누가 볼 수 있는 변화가 없어 그것만으로는 릴리스로 낼 만하지 않지만, 그 위에 올릴 것보다 먼저 끝내고 내야 합니다.
...
뒤지는 버그 찾기를 목표로 하지 않습니다**. 그것은 v0.6.11이 합니다. 알파는 모듈화뿐이고, 기능도, 누가 볼 수
있는 변화도 없습니다.

### v0.6.10 — 고급 기능: 비슷한 어떤 프로그램보다 많이

v0.6.10은 지금까지의 어느 릴리스보다 멀리 갑니다. 비슷한 프로그램들이 제공하는 기능을 모두, 그리고 그
...
지켜지게 됩니다. 그래서 v0.6.10부터는 한 저장소에서 두 판을 만들어, 같은 버전으로 동시에 냅니다.
...
옮기는 것은 업데이트가 아니라 일부러 하는 재설치입니다. Rust 코어도 두 판을 그대로 이어받습니다(v0.6.12).
...
지금은 이 제품이 어느 Codex 버전에서 동작하는지를 관리자만, 별도의 관리용 프로그램으로 기록합니다. v0.6.10부터는
누구나 보고를 보탤 수 있고, 어느 보고도 티 나지 않게 지어낼 수는 없습니다.
...
- **다른 사람의 보고는 보여 주기만 하고, 아무것도 바꾸지 않습니다.** 검증됨·확인됨과 따로, 다른 사람이 보고한
  것으로, 몇 건이 같은 결과인지와 함께 보입니다. 어떤 버전도 검증됨이나 확인됨으로 올리지 않고, 제품이 스스로 할
  수 있는 일을 바꾸지 않습니다. 그 등급은 지금처럼 관리자가 직접 얻은 근거로만 매깁니다.
...
## v0.6.11 — Python 최종 감사, 두 판 모두

v0.6.11은 마지막 Python 릴리스입니다. 저장소 전체를 두 판 모두 버그 찾기에 거치게 하고, Rust 이전이 그대로
재현할 Python 참조 구현이 됩니다.

v0.6.11은 두 판 모두에 걸쳐 Python 구현을 마지막으로 종합 감사하는 릴리스로 계획하고 있습니다.
...
## v0.6.12 — Rust: 코어를 그대로 교체하고, 그다음 Rust답게

v0.6.12도 두 단계로 나옵니다. 프리 릴리스인 **v0.6.12-alpha**는 제품의 Python 코어를 그대로 Rust로 바꾸고,
최종 **v0.6.12**는 그 코어를 자연스럽게 Rust다운 모습으로 만듭니다. 알파를 프리 릴리스로 내는 까닭은, 그대로 옮긴
코어가 Rust다운 코어로 가는 길의 한 단계일 뿐 그것만으로 낼 만한 릴리스가 아니기 때문입니다.

### v0.6.12-alpha — Rust 코어로 완전히 교체 (프리 릴리스)
...
Rust 구현은 최종 v0.6.11의 동작을 현실적으로 가능한 한 그대로 재현해야 합니다.
...
## v0.6.13 — Rust 최종 감사와 안정화: Rust 버그 찾기

v0.6.13은 마지막 종합 안정화 작업으로 계획하고 있습니다.
...
v0.6.13 이후에는 주로 유지 보수 단계로 넘어갈 것으로 봅니다.
...
v0.6.9
창의 렉
+ Codex와 함께 시작하기, 더 짧은 README

        ↓

v0.6.10-alpha → v0.6.10
Python 모듈화와 GitHub 첫 화면 정리, 프리 릴리스로
+ 비슷한 어떤 프로그램보다 많은 고급 기능을 두 판으로, 최종판으로
+ 다른 사람의 호환성 보고
+ 디자인 점검, 모양 고르기
...
이 문서는 지금의 방향을 기록합니다. 이번 릴리스는 공개되었고, 다음은 v0.6.9입니다.
