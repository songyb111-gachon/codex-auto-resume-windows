...
**UI 마무리 → 호환성 판단 추가 → Python 정리 → 선택해서 켜는 고급 복구 추가 → Python 안정화 →
...
나누기 위한 기반을 마련합니다. 실제로 나누는 작업은 v0.6.9의 알파로 옮겼습니다.
...
## v0.6.8 — 배지 없는 트레이 아이콘, 그리고 움직이는 빨간 불빛 🚧
...
실패를 알리는 빨간 불빛은 한 번 맥박친 뒤 멈춰 있었습니다. 이제 실패가 보이는 동안 계속 숨 쉽니다.
2.2초마다 한 주기로, 복구 중의 2.8초와 감시 중의 4.4초보다 조금 빠르며, 그 불빛을 보여 주는 모든 곳 - 창,
알림 카드, 트레이 아이콘, 작업 표시줄 단추 - 에서 같습니다. 확인이 필요할 때는 여전히 한 번 맥박친 뒤
머물고, 움직임 줄이기와 고대비에서는 여전히 모든 불빛이 멈춰 있습니다.

---

## v0.6.9 — Python 모듈화, 그리고 Python 최종 감사

v0.6.9는 마지막 Python 릴리스이고, 한 번에 나오지 않고 단계를 거쳐 나옵니다. 프리 릴리스인
**v0.6.9-alpha**에 모듈화가 들어가고, 아래의 나머지가 들어오고 저장소 전체가 버그 찾기를 거친 뒤에 최종
**v0.6.9**가 나옵니다. 최종판은 Rust 이전이 그대로 재현할 Python 참조 구현입니다. 그 사이에 최종판 전에
써 볼 만한 것은 각각 따로 프리 릴리스로 냅니다.

### v0.6.9-alpha — Python 모듈화 (프리 릴리스)

알파는 Python 구현의 구조를 크게 정리하는 단계로 계획하고 있습니다. 앞선 계획에서는 v0.6.5의 중심이었고,
그다음에는 v0.6.8의 중심이었습니다. v0.6.5에는 그 기반이 들어가고, 실제로 나누는 작업은 여기서 합니다.
...
최종판으로 가는 길에 v0.6.9에서는 더 약한 가정이 필요하거나, 근거가 부족하거나, Codex가 지원하지 않는 동작에 기대야 해서
...
### v0.6.9 — Python 최종 감사와 안정화

최종 v0.6.9는 Python 구현을 마지막으로 종합 감사하는 릴리스로 계획하고 있습니다.
...
## v0.6.10 — Rust 코어로 완전히 교체

v0.6.10에서는 제품에 들어가는 Python 코어를 Rust로 바꿀 계획입니다.
...
Rust 구현은 최종 v0.6.9의 동작을 현실적으로 가능한 한 그대로 재현해야 합니다.
...
v0.6.10은 동작을 똑같이 맞추는 것을 먼저 챙깁니다.

그래서 첫 Rust 구현에는 Python 모양 그대로인 구조가 어느 정도 남을 수 있습니다.

v0.6.11에서는 코드베이스를 더 자연스럽게 Rust다운 모습으로 바꿀 계획입니다.
