...
때는 하지 않으며, 동시에 시작된 두 번째는 워처 자신의 단일 인스턴스 뮤텍스에 걸려 곧바로 끝납니다. 이를 위해 Windows에 새로 등록하는 것은 없습니다. 예약 작업도,
...
지키는 기준을 지킬 수 없다는 뜻이라면, 기준에 맞춰 억지로 구부리지 않고 v0.6.11의 고급판으로 옮깁니다.
...
## v0.6.10 — Python 모듈화, 그리고 매듭지은 디자인

v0.6.10은 두 단계로 나옵니다. 프리 릴리스인 **v0.6.10-alpha**는 모듈화뿐이고, 최종 **v0.6.10**은 알파가
남긴 경계 위에서 하는 디자인 점검과 모양 고르기입니다. 프리 릴리스는 정식 버전으로 내기에는 애매하지만 개발을
...
최종 v0.6.10은 디자인을 매듭짓는 일 하나입니다. 릴리스로 낼 만하니 릴리스로 냅니다. 모든 화면의 생김새를
...
지금은 이 제품이 어느 Codex 버전에서 동작하는지를 관리자만, 별도의 관리용 프로그램으로 기록합니다. v0.6.11부터는
