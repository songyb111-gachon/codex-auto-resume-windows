...
- **제안된 액션 업그레이드 넷 중 둘은 들어갔고, 둘은 일부러 기다린다.** Dependabot이
  ...
  `checkout`과 `setup-python`은 이번 릴리스를 발행하고 확인한 뒤에 들어갔다. 아래 계획이 말하는
  순서 그대로다. 새 다이제스트 둘 다 상위 저장소의 git ref에서 직접 풀어낸 뒤에 적었고, 두 액션은
  `test.yml`이 푸시마다 실제로 돌리므로 CI가 형식이 아니라 진짜 확인이 된다. 각각 한 줄인 변경으로
  넣었다. 제안된 풀 리퀘스트는 워크플로 파일 셋을 전부 LF에서 CRLF로 다시 쓰기도 했는데,
  `.gitattributes`는 바로 그것을 막으려고 쓰여 있다.

  기다리는 둘은 아무것도 시험해 볼 수 없는 둘이다. 이번 릴리스는 빌드와 발행을 나눈 워크플로가
  처음으로 만든 릴리스다. 산출물 한 쌍은 권한 없는 빌드 작업에서 권한 있는 발행 작업으로 보관
  파일을 나르는 것이고, 증명 액션은 발행 작업이 서명하는 것이다. 그런데 둘 다 CI가 닿지 않고,
  빌드만 하고 발행 작업에는 들어가지 않는 `workflow_dispatch`도 닿지 않는다. 둘 중 어느 쪽이든
  처음 증명되는 곳은 실제 태그 푸시이고, 이 워크플로는 같은 버전을 다시 발행하기를 거부하므로 거기서
  실패하면 치르는 값은 다시 해 보기가 아니라 버전 번호 하나다. `attest-build-provenance` v4에는
  생각할 이유가 하나 더 있다. v4부터 그것은 `actions/attest`를 감싼 껍데기이고, 그 껍데기가 안에서
  고정하는 ref는 `tests/test_workflow_pins.py`가 볼 수 없다.

  하나씩 넣는다. 산출물 한 쌍을 함께(한쪽만은 결코 안 된다), 그다음 증명이며, 각각 다음 태그 전에
  `workflow_dispatch` 예행으로 확인하되, 그 예행이 정작 중요한 절반에는 닿지 못한다는 것을 알고
  한다.
