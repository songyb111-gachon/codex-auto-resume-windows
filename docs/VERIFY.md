...
- **릴리스는 GitHub의 immutable release가 아닙니다.** GitHub는 v0.5.0부터 지금까지 아홉 개 릴리스를 모두
  immutable이 아니라고 표시합니다. 이번 릴리스도 발행한 뒤에 직접 물어보았고 마찬가지였습니다. immutable release는 게시된 릴리스의 파일을 바꾸거나 태그를 옮기는
...
- **v0.5.7까지의 압축 파일은 예전 릴리스 워크플로에서 나왔습니다.** 이번 릴리스 전에 게시된 모든
  압축 파일은 action을 고정된 커밋이 아니라 움직이는 태그로 참조하는 단일 작업(job)이 빌드했고, 그
  실행 파일은 재현할 수 없습니다. 이번 릴리스는 빌드와 게시를 나눈 워크플로가 만든 첫 압축 파일이고,
...
- **제안된 액션 업그레이드 넷이 모두 들어갔고, 그중 둘은 아무것도 시험해 볼 수 없다.** Dependabot이
  ...
  이번 릴리스를 발행하고 확인한 뒤에 들어갔고, 아래 계획이 말한 순서대로 `checkout`과
  ...
  *앞*에서 돈다. 내려받아지지 않는 산출물이나 서명되지 않는 증명은 아무것도 발행하지 않은 채 실행을
  실패시킨다. 이번 릴리스의 첫 태그 푸시가 다른 이유로 정확히 그렇게 실패했다. 반쯤 만들어지는 것은
