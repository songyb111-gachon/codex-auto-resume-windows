...
## v0.6.2에서 이번 릴리스로 올리기

이번 릴리스의 압축 파일도 다른 것과 똑같이, 풀기 전에 위의 단계대로 이번 릴리스 자신의 파일과 값으로
확인합니다. `main`에 고정된 v0.6.2의 값은 v0.6.2의 파일만 보증할 뿐 다른 것은 보증하지 않습니다. 이번
릴리스가 게시되고 그 고정값이 커밋되기 전까지 3단계에는 견줄 것이 없고, 그것은 아래의
[새 릴리스에는 한동안 고정값이 없습니다](#확인되지-않은-것)가 말하는 경우입니다.
...
이번 릴리스는 기본값 하나를 일부러 바꿉니다. 작업을 이어 가려고 보내는 메시지는 중단 종류마다 고정된
...
않습니다. 이번 릴리스는 복구가 무엇을 결정하는지가 아니라 제품이 무엇을 말하고 보여 주는지를 바꾸므로,
...
- **대기 중**: 작업의 **자동 이어 가기** 확인란은 그 작업만 바꾸고, 먼저 바뀐 작업에 대한 클릭은
...
- **알림 영역 아이콘**: 한 번 클릭하면 작은 창이 열리고 한 번 더 클릭하면 닫힙니다. 다른 곳을 클릭하거나
  Esc를 누르면 닫힙니다. 그 확인란은 먼저 바뀐 작업을 거절하고, **대시보드 열기**는 창을 엽니다.
...
[SECURITY.md](https://github.com/songyb111-gachon/codex-auto-resume-windows/blob/main/SECURITY.ko.md)를
보세요.
