...
- **머리글의 불빛과 단어를 정하는 규칙은 하나입니다.** 대시보드, 팝업, 패널은 워처가 하는 일을 한 규칙으로
  말하고, 그 규칙은 표면마다 자기 코드로 돌려 보는 테스트 벡터 40개로 적혀 있습니다. 실행 중이 아니거나
  실행 중인지 알 수 없는 워처는 움직이지 않는 회색 불빛 옆에서 확인을 부탁합니다. 예전에는 팝업이 멈춘
  워처를 주황색으로 숨 쉬게 그렸습니다. 멈춘 워처 때문에 붙잡힌 기록도 그렇게 셉니다. 알림 영역 아이콘과
  작업 표시줄 단추는 자기 규칙을 지키며, 거기서는 아무도 보지 않은 실패가 빨간색입니다.
- **제품이 돌고 있다고 말하는 불빛은 모두 움직입니다.** 패널의 자동 복구 타일에는 움직이지 않던 점 대신
  ...
  약하던 일이 없어졌습니다. 패널은 대기 중인 작업의 시간이 오면 도는 호와 함께 *확인 중*이라고 말하고,
  한 번 그려진 뒤로는 워처의 다음 확인을 보지 못하므로 워처가 한 번 확인하는 동안만 그렇게 말한 뒤 다시
  *기다리는 중*이라고 말합니다.
- **머리글은 하나입니다.** 대시보드, 팝업, 알림 카드의 머리글은 패널의 것입니다. 흐린 한 줄의 제품
  이름, 그다음 불빛과 상태의 단어가 글자색으로 카드에서 가장 크게 오고, 그 아래에 사실이 옵니다. 옆에
  단어가 있는 불빛은 어느 표면에서나 줄의 시작과 단어에서 같은 거리에 섭니다. 대시보드의 머리글은 패널과
  팝업의 단어를 씁니다. *중단을 감시하는 중* 대신 *기다리는 중*이라고 말하고, 실행 중인데 확인이 필요한
  워처는 그 이유를 말합니다.
- **버튼, 칩, 콜아웃은 하나씩입니다.** 버튼은 어느 표면에서나 높이 34px에 보통 굵기입니다. 팝업과 카드의
  버튼은 32px에 굵은 글자였습니다. 칩도 높이와 여백이 하나입니다. 진단 페이지의 호환성 알림은 강조색
  글 한 덩어리 대신 패널의 콜아웃이고, 통계 그래프는 결과마다 기록이 그 결과의 단어를 쓰는 색으로
  그립니다.
- **한 가지에는 한 단어입니다.** 창은 이름이 나오는 곳마다 대시보드입니다. 아이콘의 메뉴도 팝업과 카드처럼
  *대시보드 열기*라고 말합니다. 팝업의 스위치는 대기 중 페이지의 열처럼 *자동 이어 가기*이고, 줄은
  *다음 확인*까지 세며, *기다리는 이유*의 결과는 소문자 값 단어(*통과*, *대기*, *막힘*, *알 수 없음*)입니다.
  아홉 언어 모두 그렇습니다.
- **카드의 불빛은 덜 듭니다.** 팝업이 자기 불빛을 그리듯 불빛이 선 줄의 띠만 다시 그리므로, 프레임은
  두 배쯤이고 프로세서 시간은 4분의 1에서 3분의 1 사이입니다.
...
- 대시보드의 설정 > 모양에 있는 **디자인**은 대시보드, 팝업, 알림 카드, Codex 안의 패널을 네 가지 가운데
  ...
  *단순하게*입니다. [BRAND.ko.md](BRAND.ko.md#네-가지-디자인)에 정리했고,
