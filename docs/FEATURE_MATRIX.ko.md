...
- **이번 릴리스는 아직 공개되지 않았고, 그 앞의 두 릴리스는 공개되어 있습니다.** 아래에서
  ...
  *않는* 것은, 누군가 그것을 설치해서 써 보았다는 것입니다. 발행된 바이트는 그 확인만을 위해 만든
  ...
  일은 [LIVE_ACCEPTANCE.ko.md](LIVE_ACCEPTANCE.ko.md)가 맡습니다. 현재 그 절차를 진행 중이며
  `docs/evidence/live/`에 완료한 단계와 막힌 단계를 구분했습니다. 새로 받은 검사 대상 릴리스은 게시된 체크섬과
...
- **REAL WINDOWS TESTED 항목은 대개 진짜 Windows 기능이 필요한 스위트 안의 테스트를 뜻하고**,
  ...
  REAL WINDOWS TESTED 줄은 그 이전 실행에서 돌았으며 라이브 검사는 돌지 않았다. 새로 진행한
