...
- **이번 릴리스는 아직 공개되지 않았고, 공개된 가장 새 릴리스는 v0.6.3입니다.** 아래에서 PUBLISHED라고
  ...
  다시 돌려 보지 않았기 때문입니다. 이번 릴리스는 대시보드와 팝업을 패널과 같은 재질로 옮기고, 상태
  불빛을 차분하게 바꾸고, 창을 v0.6.2의 크기로 더 빠르게 열고, 선택 칸의 잘린 아래쪽 테두리를
  고칩니다. 이번 릴리스가 공개되기 전까지는 이 문서의 어느 것도 그에 대해 PUBLISHED라고 하지 않습니다. 이 문서 어디에서도 PUBLISHED가 뜻하지
...
| 화면 언어 아홉 개 - 영어, 한국어, 일본어, 중국어 간체, 중국어 번체, 스페인어, 독일어, 프랑스어, 브라질 포르투갈어 - 가 언어마다 JSON 카탈로그 하나이고, 영어가 원본이자 번역이 아직 닿지 않은 키의 대체다 | UNIT TESTED | `tests/test_l10n.py:ShippedCatalogTests`(5개), `LoaderTests`(5개, `test_a_missing_key_is_english_and_a_present_one_is_translated`, `test_a_broken_translation_falls_back_to_english_instead_of_failing` 포함), `NoNetworkTests.test_localization_reaches_no_network`; `tests/test_locale.py:CatalogTests`(9개), `ReachTests.test_one_language_reaches_all_four` | 여덟 번역 가운데 어느 것도 그 언어를 모국어로 쓰는 사람이 검토하지 않았다. 빠진 키, 자리표시자, 최신 여부는 확인하지만 문장은 확인하지 않는다. |
...
| 창은 v0.6.2의 크기인 860 × 600으로 열리고, 가장 긴 설정 부분에 맞춰 커지지 않는다. 스크롤되는 쪽은 모든 배율에서 카드의 그림자가 들어갈 자리를 남긴다 | UNIT TESTED | `tests/test_gui_layout.py:WindowCompositionTests.test_the_window_opens_at_the_proportions_of_v062_and_does_not_grow`, `test_a_scrolling_page_keeps_room_for_a_cards_lift` | 소스 모양 확인이다. 크기와 여백을 `gui/SettingsApp.cs`에서 읽고, 여백을 다섯 가지 배율에서 brand의 그림자가 닿는 거리와 견준다. 설정 부분을 스크롤해 보는 테스트는 없고, 그 크기로 화면에 열린 창을 기록한 것도 없다. |
...
| 창은 패널과 같은 재질로 그린다. 창이 찍는 그림자는 100%, 150%, 200%에서 모두 255단계 가운데 2단계 안으로 `brand.shadow_alpha`의 가우스 분포이고, 합성한 카드·컨트롤·안으로 들어간 칸은 brand의 모형과 맞으며, 긴 가장자리에는 이음매가 없고, 바탕은 brand가 주는 닿는 거리만큼을 비워 두며, 카드는 그림자용 띠를 두지 않고, 숫자 칸은 진짜 스핀 상자를 둘러싼 칸이며, 기본 버튼의 마우스 올림·누름 색은 brand의 것이다 | REAL WINDOWS TESTED | `tests/test_gui_v064.py:MaterialTests`(10개)가 창을 컴파일하고, 창 자신의 그리기 코드가 비트맵에 그림자를 찍어 리플렉션으로 답하게 한다. `SourceRuleTests`(5개)는 unsafe 코드와 초기값 있는 상수 배열을 거부하고, 모든 바탕과 떠 있는 컨트롤이 모서리 뒤를 칠하며 바탕이 이중 버퍼에 불투명하기를 요구한다 | 내장 컴파일러나 PowerShell이 없으면 건너뛰고, 고대비를 쓰는 기계에서는 버튼 색도 건너뛴다. 그림자는 조합마다 각 가장자리의 가운데를 가로질러, 그리고 합성 하나로 견줄 뿐 쪽 전체를 패널과 견주지는 않는다. 마우스 올림, 누름, 키보드 초점은 테스트가 코드에 묻는 것일 뿐 본 적이 없고, 그것을 보여 주는 캡처도 없다. |
...
| 배치: 모든 배율과 모든 언어에서 누를 곳이 모두 카드 안에 있고, 줄바꿈한 글에는 잰 만큼의 높이를 주고, Tab과 Shift+Tab으로 돌며, 작업마다 스위치가 이름표 첫 줄의 시작에 놓이고, 머리글은 제자리에 있고 빛은 카드 안에 머물며, 카드의 그림자는 창 가장자리에 이르기 전에 바탕으로 사라지고, 문자 체계마다 글꼴이 있다 | UNIT TESTED | `tests/test_tray_popup.py:LayoutTests`(11개), `FontTests`(4개) | 글자는 진짜 렌더러가 아니라 글자마다 폭이 고정된 대역으로 잰다. |
...
| 브랜드 자산과 아이콘이 원본에서 생성되고 최신 상태로 유지되며, `gui/Brand.cs`는 배치 크기, 그림자 조합, 빛의 수치를 낱낱의 상수로 싣고, 내장 컴파일러가 무작위로 이름 짓게 될 배열은 싣지 않는다 | UNIT TESTED | `tests/test_brand.py:GeneratedFileTests`(5개), `PaletteTests`(5개), `SurfaceTests`(6개), `GeneratedWindowTokenTests`(2개) | - |
...
| `gui/Brand.cs`로 생성한 창의 상태 불빛은 `brand.py`의 것이다. 컴파일해서 부르면 빛, 타이머 규칙, 점의 색이 `brand.glow`, `brand.glow_moves`, 팔레트와 같은 답을 내고, 생성된 상수는 모두 `brand.py`가 쓴 값으로 다시 읽힌다 | REAL WINDOWS TESTED | `tests/test_brand.py:GeneratedStatusLightTests`(4개)가 내장 컴파일러로 `gui/Brand.cs`를 컴파일하고, 상태 열 가지를 시점 열네 곳과 진입 시각 일곱 가지에서, 움직임 줄이기를 끄고 켜며 리플렉션으로 부른다 | 내장 컴파일러나 PowerShell이 없으면 건너뛰고, 그런 곳에서는 생성된 규칙이 `brand.glow`에서 멀어져도 알아차리는 것이 없다. `GeneratedFileTests.test_gui_brand_cs_is_current`는 파일이 생성기가 쓰는 그대로라는 것만 보이고, 생성기는 그 규칙의 사본을 따로 들고 있다. |
...
가장 얇은 쪽은 표면입니다. 창의 여섯 쪽은 진짜 코드지만 캡처는 셋뿐이고 모두 v0.6.3의 새
모양보다 앞서며, 알림 영역 아이콘과 새 팝업은 몇 개의 테스트에서 화면 밖에 진짜로 만들어졌다
사라질 뿐 어느 쪽도 그림 한 장 없고, Codex 패널은 Codex가 한 번도 불러온 적이 없습니다. v0.6.3의
요청 함수가 Node에서 돌 뿐입니다. 표면이 제어 계층과 이야기하는 곳에서는, 제어 계층은 잘 시험되어
