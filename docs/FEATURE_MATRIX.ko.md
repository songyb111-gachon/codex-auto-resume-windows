...
| 직접 입력 메시지는 번역하지 않고 쓴 그대로 보내며, `{reason}`, `{category}`, `{attempt}`, `{max_attempts}`, `{reset_time}`만 쓸 수 있고, 2000자를 넘지 않으며, 중단 종류별 메시지가 비면 모든 중단에 쓰는 메시지로, 그것도 비면 기본 메시지로 대신한다 | UNIT TESTED | `tests/test_continuation.py:CustomMessageTests`(10개, `test_forbidden_and_unknown_placeholders_are_refused_by_name`, `test_length_and_emptiness`, `test_the_fallback_order_is_per_reason_then_global_then_standard`, `test_custom_text_can_never_be_attached_to_a_failure_that_is_not_recovered` 포함), `NoLeakageTests.test_nothing_from_the_record_but_the_safe_fields_reaches_the_text` | 문장의 내용은 쓴 사람의 것이다. 자리표시자는 제한되지만 나머지 문장은 그렇지 않고, 직접 입력 메시지가 Codex가 일을 잇는 데 도움이 되는지는 아무것도 확인하지 않는다. |
| 미리보기는 워처가 보낼 문장을 같은 함수로 만든 것이며, 중단 종류마다, 아직 저장하지 않은 선택에 대해서도 보여 주고, 아무것도 저장하지 않는다 | UNIT TESTED | `tests/test_control_continuation.py:PreviewTests`(9개, `test_the_preview_is_the_text_the_watcher_builds`, `test_unsaved_choices_are_previewed_and_nothing_is_saved`, `test_a_refused_custom_message_shows_what_would_really_be_sent` 포함). `BridgeCommandTests.test_the_window_reaches_all_three`가 브리지를 거쳐 닿는다 | 미리보기에는 워처가 문장 뒤에 덧붙이는 표지 줄이 빠져 있고, 미리보기가 그렇다고 말한다. 대시보드의 미리보기도 패널의 미리보기도 사람이 쓰는 방식으로 구동하지는 않는다. |
...
| 영어에서 옮긴 뒤 영어가 바뀐 번역은 다시 살펴볼 때까지 낡았다고 보고되고, 자리표시자를 잃거나 모르는 키를 더한 가져오기는 통째로 거절된다 | UNIT TESTED | `tests/test_l10n.py:ToolTests`(3개), `ShippedCatalogTests.test_every_translation_is_complete_and_current` | 이 도구는 `build/` 아래에 있고, 릴리스 압축 파일에는 들어가지 않는다. |
...
| 진단 내보내기는 가려 쓴 파일을 쓰고 기존 파일을 덮지 않는다 | UNIT TESTED | `tests/test_diagnostics.py:DiagnosticsTests`(7개) | 시험되는 것은 `diagnostics.write`뿐이다. 버튼, 파일 선택, 보내기 전에 읽어 보는 흐름은 확인되지 않았다. |
...
| 진단: 식별할 수 있는 것이 남지 않고, 별칭은 한 파일 안에서는 일관되고 파일끼리는 다르며, 직접 입력 메시지는 설정되었다는 사실만 적고 문장은 옮기지 않으며, 기존 파일을 덮지 않는다 | UNIT TESTED | `tests/test_diagnostics.py:DiagnosticsTests`(7개, `test_a_custom_message_is_recorded_as_set_and_never_quoted` 포함) | 만들어 낸 로그를 상대로 돌 뿐, 실제 사고에서 나온 묶음을 상대로 하지 않는다. |
...
| 브랜드 자산과 아이콘이 원본에서 생성되고 최신 상태로 유지된다 | UNIT TESTED | `tests/test_brand.py:GeneratedFileTests`(4개), `PaletteTests`(5개), `SurfaceTests`(6개) | - |
