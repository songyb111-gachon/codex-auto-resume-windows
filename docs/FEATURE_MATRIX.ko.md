...
| 머리글의 상태 점은 워처가 무엇을 하는지 말한다 - 감시 중, 기다리는 중, 때가 된 작업을 확인 중, 복구 중, 일시 중지됨, 확인이 필요함 - 일하는 동안에만 되풀이해 움직이고, 경보에는 한 번만 깜박이며, 움직임을 줄이면 전혀 움직이지 않는다 | REAL WINDOWS TESTED | `tests/test_gui_v063.py:AliveStateTests`(6개)가 내장 C# 컴파일러로 창을 컴파일한 뒤, 워처 상태와 기다리는 일의 조합 열한 가지와 여섯 시점에 대해, 움직임 줄이기를 끄고 켜며, 창 자신의 정적 판단·불투명도 메서드를 리플렉션으로 부른다 | 내장 컴파일러나 PowerShell이 없으면 건너뛴다. 메서드를 부를 뿐 창을 띄우지는 않고, 움직이는 머리글을 찍은 캡처도 없다. 움직임 줄이기는 인자로 바로 넘기므로, 이 테스트들이 바꾸어 보는 것은 Windows 자체의 애니메이션 설정이 아니다. |
