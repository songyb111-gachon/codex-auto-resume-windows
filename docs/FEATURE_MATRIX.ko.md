...
- **REAL WINDOWS TESTED 항목은 대개 진짜 Windows 기능이 필요한 스위트 안의 테스트를 뜻하고**,
  ...
  이번 수락 감사에서는 기준 검사의 통과가 놓친 안전성·설치기·MCP/UI 결함도 재현했고, 그 뒤
  평범하게 쓰는 중에 하나가 더 나왔다. 성공한 복구가 실패로 기록되고 있었다. 그 수정들이 이번
  릴리스이며, 검사 대상이었던 게시본의 바이트에는 들어 있지 않다. 고침에는 새 버전을 준다.
  그 게시본에 대한 안전성 주장은 모두 이 결함들과 아직 끝나지 않은 실제 수락 검사를 함께 놓고
  읽어야 하고, 변경 기록의 맨 위 절이 있는 이유가 그것이다.
