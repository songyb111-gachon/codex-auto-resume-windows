...
- **REAL WINDOWS TESTED 항목은 대개 진짜 Windows 기능이 필요한 스위트 안의 테스트를 뜻하고**,
  ...
  기록을 위해 적어 둡니다. 이 문서를 쓴 Windows 11 기계에서 2026-09-12에
  `PYTHONPATH="src;tests" python -m unittest discover -s tests`를 돌린 결과는 **1140개 테스트, 310초,
  실패 두 개, 건너뛴 것 여섯 개**였습니다. 빨간불이었습니다. 실패 둘은 이 문서가 설명하는 것이 아니라
  한국어 브랜치 쪽 정리입니다.
  `tests/test_korean.py:MappingTests.test_every_korean_document_is_mapped`는 이 문서와
  `docs/LIVE_ACCEPTANCE.md`의 한국어 짝이 아직 `scripts/ko_branch.json`에 올라 있지 않아서 실패하고,
  `test_no_translation_is_stale`은 `README.md`, `CONTRIBUTING.md`, `docs/DEVELOPMENT.md`가 그 두
  문서로 가는 링크를 얻으면서 한국어 짝의 검토 지점보다 새로워졌기 때문에 실패합니다. 매핑을 더하고
  `python scripts/ko_sync.py --reviewed <영어 문서 경로>`를 돌리면 둘 다 사라지며, 제품 쪽은 아무것도
  건드리지 않습니다. 건너뛴 여섯 개는 모두 `tests/test_integration_live.py`의 선택형 라이브 점검이었고,
  하나같이 `CODEX_AR_LIVE=1` 없이는 돌지 않습니다. 즉 아래의 REAL WINDOWS TESTED 항목은 전부 여기서
  실제로 실행되었고, 실제 Codex 설치를 건드리는 점검은 하나도 실행되지 않았습니다.
...
| 지금 다시 확인이 워처를 깨운다 | IMPLEMENTED | `src/codex_auto_resume/windows.py:WakeEvent`. 제어 계층은 깨울 것이 있었는지 답하고, `tests/test_control_v3.py:RetryNowTests`가 "깨울 워처가 없다"는 답을 덮는다 | **진짜 깨우기 이벤트에 신호를 보내고, 기다리던 워처가 기다림을 줄이는 것을 확인하는 테스트가 없다.** 덮여 있는 경로는 이벤트를 쓸 수 없을 때뿐이다(`tests/test_cli.py:WatcherLoopTests`). |
...
| 읽을 수 없는 조각은 빈 목록이 아니라 읽을 수 없음으로 보인다 | IMPLEMENTED | `gui/Dashboard.cs`(`ShowUnreadable`, `MarkUnavailable`, `ClearRecent`) | **창에서는 확인되지 않았다.** C#이 그 갈래로 가는지 확인하는 것이 없고, 그 화면의 스크린샷도 없다. 브리지를 일부러 실패시키고 한 장 찍으면 해결된다. |
...
| 지금 다시 확인은 할 일이 있는 곳에만, 시도 되돌리기는 보낼 수 없는 곳에 제공된다 | IMPLEMENTED | `gui/Dashboard.cs`(`UpdatePendingButtons`, `UpdateHistoryButtons`). 그 조건이 피하려는 거절은 모두 제어 계층에서 시험된다: `tests/test_control_v3.py:RefusalCodeTests.test_each_retry_now_refusal_arrives_as_its_own_code`, `test_each_restore_budget_refusal_arrives_as_its_own_code` | **버튼을 켜고 끄는 논리 자체가 확인되지 않았다.** 조건이 틀리면 눌렀다가 거절당하는 버튼을 내놓게 된다. 안전하기는 하지만, 창이 못 할 일을 약속한 셈이다. |
...
| 설치 복구는 다섯 가지 중 무엇이 일어났는지 말하고, 설치 관리자의 잠금을 잡으며 `--keep-state`로 돈다 | IMPLEMENTED | `gui/Dashboard.cs`(`Repair`, `RunRepair`, `ReportRepair`). 다투는 그 잠금은 `tests/test_convergence.py:InstallerLockTests`(3개)가, `--keep-state`는 `tests/test_plugin.py:BridgeTests.test_keep_state_repairs_an_autostart_that_is_already_ours`가 시험한다 | **다섯 결과 중 하나도 실행되지 않는다.** "이미 실행 중", "아직 작업 중", "파일이 없음", 그리고 잘라 낸 실패 꼬리는 확인되지 않았다. 가짜 setup 프로세스로 `RunRepair`를 다섯 조건마다 돌리면 덮인다. |
...
| 로컬 웹 서버가 없고, 브라우저로 열리는 것도 없다 | IMPLEMENTED | `tests/test_privacy_claims.py:CodePropertyTests.test_the_recovery_runtime_imports_no_networking_module`이 `src/`와 `scripts/*.py`를 덮는다 | **`gui/*.cs`는 그 확인에 들어 있지 않다.** 창이 소켓을 열지 않고 브라우저를 띄우지 않는다는 것을 확인하는 것이 없다. 같은 검사를 C# 소스까지 넓히면 닫힌다. |
...
| 메뉴는 창을 열고, 자동 복구를 일시 정지하거나 다시 켜고, 워처를 멈춘다. 모두 같은 제어 계층을 지난다 | IMPLEMENTED | `src/codex_auto_resume/tray.py` | **확인되지 않았다.** 메뉴 명령을 보내거나 처리기가 제어 계층에 닿는지 확인하는 테스트가 없다. 메뉴 명령 id를 따라가는 테스트가 가장 작은 해결책이다. |
...
| 알림이 실제로 그려지는 것은 시작 메뉴 바로 가기가 같은 AppUserModelID를 싣고 있기 때문이다 | UNVERIFIED | `src/codex_auto_resume/shortcut.py`가 셸 자신의 COM 인터페이스로 그 바로 가기를 쓴다. `System.AppUserModel.ID`를 붙이려면 `IPropertyStore`가 필요해서 PowerShell로 구동한다. 그 모듈의 docstring이 Windows 11에서 잰 것을 적고 있다. 바로 가기가 없으면 플랫폼은 알림을 받아들이고 전달했다고 기록까지 하면서 그리지만 않는다 | **스위트 안에서 진짜 `.lnk`를 쓰는 것은 하나도 없고**, 설치나 제거를 지나가는 테스트는 모두 `shortcut.install`과 `shortcut.uninstall`을 가짜로 바꿔치기한다(`tests/test_cli.py`, `tests/test_plugin.py`, `tests/test_installer.py`). 그 측정은 docstring에 적혀 있을 뿐 산출물로 남아 있지 않아서, 읽는 사람에게는 이 문장뿐이다. Windows에서 임시 폴더에 바로 가기 하나를 써 보고 그 id를 다시 읽어 보는 것이 가장 작은 해결이다. |
...
| 깨우기 이벤트 덕분에 지금 다시 확인이 기다림을 줄인다 | IMPLEMENTED | `src/codex_auto_resume/windows.py:WakeEvent`. 이벤트를 쓸 수 없는 경우의 대체 경로는 `tests/test_cli.py:WatcherLoopTests`가 덮는다 | **확인되지 않았다.** 진짜 깨우기 이벤트에 신호를 보내고 워처가 반응하는 것을 확인하는 테스트가 없다. |
| 홈 잠금이 한 상태 디렉터리에 프로세스 둘이 들어가는 것을 막는다 | IMPLEMENTED | `src/codex_auto_resume/windows.py:HomeLock`. `home_lock_unavailable`은 `src/codex_auto_resume/machine.py`의 정식 거절 사유다 | **확인되지 않았다.** 스위트 전체에서 `HomeLock`을 이름으로 부르는 테스트가 하나도 없다. |
...
| 부트스트랩은 실행 전에 검증하고, GitHub 호스트만 받아들이고, 어떤 버전을 받을지 지시받을 수 없고, 실패하면 아무것도 남기지 않는다 | UNIT TESTED | `tests/test_convergence.py:BootstrapTests`(7개), `ReleaseManifestTests`(6개), `HostPortabilityTests`(3개) | `scripts/bootstrap.ps1`에 대한 소스 확인이다. 스위트에서 내려받기는 일어나지 않는다. |
...
| 일지를 다시 읽어 무엇을 결정하는 곳은 없다 | IMPLEMENTED | 결정은 레코드에서 내린다. `src/codex_auto_resume/store.py`의 `events()`는 제어 계층에서만 닿는다 | **속성으로서는 확인되지 않았다.** `tests/test_control.py:NotARecoveryEngineTests` 같은 소스 모양 테스트로 엔진이 `events()`를 부르지 않는다고 못 박으면 닫힌다. |
...
| GUI 자동화, 마우스·키보드 흉내, OCR, 화면 긁기, 접근성 API 조작을 쓰지 않는다 | IMPLEMENTED | README가 그렇게 적고 있고, 살펴본 결과 제품 안의 유일한 `user32` 호출은 `src/codex_auto_resume/tray.py`의 `SetForegroundWindow`로, 이 제품 자신의 창을 앞으로 가져오는 데 쓴다 | **확인되는 속성이 아니다.** 나중에 누가 `SendInput`이나 `keybd_event`나 UI Automation import를 넣어도 실패하는 것이 없다. 네트워크 검사 옆에 금지 목록 테스트를 두면 README의 주장이 검토가 아니라 강제가 된다. |
...
| 두 실행 파일이 플러그인 매니페스트에서 만든 버전 자원을 싣는다 | UNIT TESTED, 빌드 산출물이 함께 있음 | `tests/test_reproducible.py:BuildScriptTests.test_the_executables_carry_a_version_resource_from_the_manifest`. `build/CodexAutoResumeSettings.exe.VersionInfo.cs`와 `build/codex-auto-resume-mcp.exe.VersionInfo.cs`가 이 나무에 있는 생성 산출물이다 | 빌드된 실행 파일의 속성·자세히를 열어 값을 읽어 본 기록이 없다. |
...
| --- | --- | --- |
| 홈 잠금 | IMPLEMENTED | 한 디렉터리에 `HomeLock` 둘을 열어 두 번째가 거절되기를 요구하는 테스트. |
| 깨우기 이벤트 | IMPLEMENTED | 진짜 깨우기 이벤트에 신호를 보내고 기다리던 워처가 기다림을 줄이는 것을 확인하는 테스트. |
| 알림 영역 메뉴 항목 | IMPLEMENTED | 메뉴 명령 id를 하나씩 보내 제어 계층에 닿는지 확인하는 테스트. |
| 창의 설치 복구, 다섯 결과 전부 | IMPLEMENTED | 다섯 조건을 돌려주는 가짜 setup 프로세스로 `RunRepair`를 구동. |
| 창의 "빈 목록이 아니라 읽을 수 없음" | IMPLEMENTED | 브리지를 일부러 실패시키고 대기 중 쪽을 한 장 캡처. |
| 창이 어떤 버튼을 내놓는가 | IMPLEMENTED | 상태별 레코드를 상대로 `UpdatePendingButtons`와 `UpdateHistoryButtons`를 확인하는 테스트. |
| 5초 갱신 | IMPLEMENTED | 그것을 재는 무엇이든. |
| C#에도 웹 서버·브라우저가 없다는 것 | IMPLEMENTED | `tests/test_privacy_claims.py`의 코드 속성 검사를 `gui/*.cs`까지 확대. |
| GUI 자동화를 절대 쓰지 않는다는 것 | IMPLEMENTED | `SendInput`, `keybd_event`, UI Automation, OCR에 대한 금지 목록 테스트. |
| 일지를 다시 읽어 결정하지 않는다는 것 | IMPLEMENTED | 엔진이 `events()`를 부르지 않는다는 소스 모양 테스트. |
...
| 시작 메뉴 바로 가기, 그리고 그것에 기대는 알림 이름 | UNVERIFIED | Windows에서 임시 폴더에 `.lnk` 하나를 쓰고 AppUserModelID를 다시 읽어 보기. |
