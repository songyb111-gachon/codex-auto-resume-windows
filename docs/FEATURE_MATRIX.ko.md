...
- **이번 릴리스는 아직 공개되지 않았고, 공개된 가장 새 릴리스는 v0.6.2입니다.** 아래에서
  PUBLISHED라고 적힌 항목들이 뜻하는 것은 공개된 릴리스, 곧 v0.6.0, v0.6.1, v0.6.2입니다. 셋 다
  압축 파일이 발행되었고 해시가 `main`에 고정되어 있습니다. 이번 릴리스는 화면 언어 아홉 개,
  이어가기 메시지 설정, 알림 영역 아이콘의 팝업, 알림의 대시보드 열기 단추, 새로 꾸민 창과 패널을
  더하고, 공개된 세 버전 모두에 있던 결함 하나 - 복구되는데도 끌 스위치가 없던 중단 종류 - 를
  고칩니다. 공개되기 전까지 그 가운데 무엇에 대해서도 PUBLISHED를 주장하지 않습니다. 이 문서 어디에서도 PUBLISHED가 뜻하지
