...
**모든 화면은 한 벌의 기록을 한 시각에 그립니다.** 창의 예시 기록(`seed_window_state`)을 팝업과
패널을 그리는 바로 그 시각에 한 번 써 두고, 화면마다 자기가 읽는 방식 그대로 다시 읽습니다. 팝업의 행과
패널의 행은 창에 보이는 대기 중인 복구 두 건이고, 이름과 상태와 시각이 같습니다. 패널 페이지에도 그
시각을 알려 주고, 카드처럼 시계 시각을 UTC로 읽게 합니다. 그래서 카운트다운과 칩과 개수가 네 그림 모두에서
같은 말을 합니다.
