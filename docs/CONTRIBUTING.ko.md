...
보고서는 내가 여는 풀 리퀘스트로만 GitHub에 가며, 그 풀 리퀘스트는 새 파일 정확히 하나를 다음 자리에
더합니다.

    docs/evidence/community/<내 GitHub 로그인>/codex-cli-<버전>.json

`<버전>`은 보고서 자신의 `codex_version`이고, 로그인은 풀 리퀘스트를 여는 계정의 것(그리고 보고서의
`reporter.github_login`)입니다. 보고서는 더하기만 합니다. GitHub 로그인 하나와 Codex 버전 하나에 보고서
...
목록은 [SECURITY.ko.md](SECURITY.ko.md)에 있습니다). 파일은 1 MB 이하여야 하고, 보고서 형식의 키만 정확히
...
고친 결론은 남지 않습니다. 모든 보고서는 여전히 관리자가 검토하고 병합합니다.

접수된 보고서는 다음 릴리스부터 대시보드 진단 페이지의 Codex 버전 옆 *다른 사람들의 보고*에 셈해집니다(어떻게
세는지는 [GUIDE.ko.md](GUIDE.ko.md#요구-사항)에 있습니다). 관리자의 도구가 그것을
`docs/evidence/community/index.json`에 더하고 횟수를 `src/codex_auto_resume/data/reported.json`에 쓰며,
`tests/test_reported_data.py`가 파일과 색인과 함께 나가는 횟수를 서로 맞춰 붙잡습니다. 색인에 없는 보고서
파일은 어디에도 셈해지지 않으므로, 내 풀 리퀘스트는 일반 테스트에서 초록으로 남습니다. 틀린 것으로 드러난
보고서는 공개적으로, 파일과 그 색인 항목을 함께 지워서 거둬들입니다. 어떤 보고서도 호환성 데이터에 들어가지
않으며, 어떤 보고서도 등급을 바꾸지 않습니다.
