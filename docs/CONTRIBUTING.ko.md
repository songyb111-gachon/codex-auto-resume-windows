...
채 14일 동안 바뀌지 않으면 닫습니다. 저장소 변수 `COMMUNITY_AUTOFILE`로 접수를 멈추고(비어 있거나 `on`이
아니면 무엇이든), `COMMUNITY_BLOCKED`에 계정의 숫자 id를 적어 그 계정의 보고서를 거부하며, 관리자 자신의
도구로는 기다리는 보고서를 접수하거나 보고서를 거둬들일 수 있습니다.

접수된 보고서는 다음 릴리스부터 대시보드 진단 페이지의 Codex 버전 옆 *다른 사람들의 보고*에 셈해집니다(어떻게
세는지는 [GUIDE.ko.md](GUIDE.ko.md#요구-사항)에 있습니다). 그것을 접수하는 커밋이
...
거둬들이고, 그 계정을 `docs/evidence/community/withdrawn.json`에 적어 같은 보고서가 다시 접수되지 않게 합니다.
