...
v0.6.3 설치본도 tool은 열일곱 개로 같지만, 그 `update_settings`는 테마를 제시하지 않습니다. v0.6.0부터 v0.6.2까지의 설치본은 tool이 열여섯 개로 `preview_recovery_message`가 없고, `update_settings`가 제시하는 것도 복구 분류,
...
줍니다. 또 `update_settings`는 Windows에 속한 두 설정, 알림 영역 아이콘(`show_tray`)과 움직임 줄이기(`reduce_motion`)도
제시하지 않으며, 패널도 이 둘을 보여 주지 않습니다. 이번 릴리스부터는 패널이 보여 주는 유일한 모양 설정인 테마를
제시합니다.
...
대기 중 페이지를 열 뿐 다른 일은 할 수 없는 **대시보드 열기**가 담깁니다.
