...
아래 표는 이번 릴리스부터의 서버를 설명합니다. 서버는 사용자가 추가한 플러그인이 아니라 설치된 릴리스에서 실행됩니다.
...
v0.6.4부터는 테마를, 이번 릴리스부터는 그 옆의 패널 자신의 테마(`panel_theme`)도 제시합니다. 패널이 보여
...
그려지며, Codex의 밝은 테마와 어두운 테마, 밝게, 어둡게, 또는 기본값인 테마 설정의 선택(이전 릴리스까지 패널이 따르던
