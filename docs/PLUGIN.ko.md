...
했는지를 `logs/codex-start.log`에 한 줄로 적습니다. `start_watcher`가 요청을 받아 시작하는 워처도 같은 작업
개체 안에서 돌므로, 이번 릴리스부터 그 응답은 워처가 Codex를 닫을 때(그보다 일찍일 수도 있음) 멈춘다고
말합니다. 워처가 다음에 레코드를 *언제* 볼지를 바꾸는 tool이 두 가지 있습니다. `retry_now`는 대기
...
| `start_watcher` | 워처가 돌고 있지 않으면, 설치기가 띄우는 것과 같은 워처를 띄웁니다. 이렇게 띄운 워처는 Codex 안에서 돕니다. Codex가 플러그인이 시작한 것을 끝내는 곳에서는(Codex 26.915가 그렇다는 것을 쟀습니다) 이번 릴리스부터 응답이 워처가 Codex를 닫을 때(그보다 일찍일 수도 있음) 멈춘다고 말하고(`ends_with_codex`, 매번 작업 개체를 읽어 정합니다), Codex보다 오래 도는 시작을 알려 줍니다. Codex를 닫은 뒤 시작 메뉴의 Codex Auto Resume에서 시작하거나, 대시보드에서 Windows 로그인 시 실행을 켜는 것입니다. 그 워처의 아이콘에서 연 대시보드도 Codex 안에서 돕니다. | 예 |
