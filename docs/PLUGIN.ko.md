...
| `start_watcher` | 워처가 돌고 있지 않으면, 설치기가 띄우는 것과 같은 워처를 띄웁니다. 이렇게 띄운 워처는 Codex 안에서 돌고, Codex 26.915 이후는 플러그인이 시작한 것을 끝내므로, 이번 릴리스부터 응답은 워처가 Codex를 닫을 때(그보다 일찍일 수도 있음) 멈추는 경우 그렇다고 말하고(`ends_with_codex`), 워처를 계속 돌게 하는 대시보드의 워처 시작이나 로그인 시 시작을 가리킵니다. | 예 |
