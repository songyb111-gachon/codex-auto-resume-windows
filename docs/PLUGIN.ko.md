...
그리고 v0.6.4 다음 릴리스부터의 알림 카드(`notification_card`)도 제시하지 않으며, 패널도 이것들을 보여 주지 않습니다.
v0.6.4부터는 패널이 보여 주는 유일한 모양 설정인 테마를 제시합니다. Codex 호환성 데이터를 새로 받거나 가져오는
...
그리므로, 자기만의 목록이 아니라 설정 모듈이 정의한 필드를 보여 줍니다. 테마 설정으로 따로 고르지 않는 한 Codex의 밝은 테마와
어두운 테마를 따르고, Codex의 움직임 줄이기 선호를 따르며, 대시보드와 팝업이 함께 쓰는 시각 언어로 그려집니다([BRAND.ko.md](BRAND.ko.md)). 위에서 아래로:
...
* **모양**: 테마(시스템 설정 따르기, 밝게, 어둡게). 새 테마를 저장하면 패널이 곧바로 그 테마로 그려집니다.
* **저장**: 위 설정 가운데 저장을 기다리는 것들을 위한 단추입니다. 화면 언어와 테마는 패널에서 바꾼 경우에만
...
| | |
| --- | --- |
| **어디서** | URL 형태 하나뿐이며, `scripts/release.json`과 이 플러그인 자신의 매니페스트에 적힌 버전으로 만듭니다. "latest"도 없고, URL에 닿는 파라미터도 없습니다. 평소 실행에서 특정 버전의 플러그인은 그 버전의 압축 파일과 그 옆에 게시된 `.sha256`만 내려받습니다. `.sha256`은 대조할 고정 digest가 없을 때만 내려받습니다. `-Update`는 하나뿐인 예외이며, 그때 내려받는 버전도 입력이 아니라 바로 이 소유자와 저장소 아래의 리디렉션에서 읽은 정수 세 개이고, 아래의 검사는 모두 그대로 적용됩니다. v0.6.4 다음 릴리스부터는 요청할 때만 내려받는 것이 하나 더 있습니다. `-Compatibility`로, 또는 github.com이 답한 업데이트 확인(`-CheckOnly`, `-Update`) 뒤에 받는 Codex 호환성 데이터입니다. 고정된 주소 하나, 곧 이 저장소 `main`의 `src/codex_auto_resume/data/codex_compat.json`으로 보내는 `GET` 한 번이며, 받은 것은 설치본 자신의 검증기에 넘길 뿐 실행하지 않습니다. |
...
대기 중 페이지를 열 뿐 다른 일은 할 수 없는 **대시보드 열기**가 담깁니다. v0.6.4 다음 릴리스부터는 카드를
