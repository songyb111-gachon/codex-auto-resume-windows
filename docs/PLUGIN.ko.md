...
계속 돕니다. v0.6.8 다음 릴리스부터 서버는 답하는 일 말고 한 가지를 더 합니다. 대시보드에서 **Codex가
시작될 때 함께 시작**을 켜 두었다면, 워처가 돌고 있지 않을 때 Windows 로그인이 쓰는 그 실행기로 워처를
시작합니다. tool이 아니라 사람이 정해 둔 선택이며(어떤 tool도 이것을 시작하지 않고 `update_settings`는 이
설정을 거절합니다), 어떤 복구에 대해서도 아무것도 정하지 않습니다. 워처가 다음에 레코드를 *언제* 볼지를 바꾸는 tool이 두 가지 있습니다. `retry_now`는 대기
...
v0.6.5부터의 알림 카드(`notification_card`), v0.6.8 다음 릴리스부터의 **Codex가 시작될 때 함께 시작**
(`start_with_codex`)도 제시하지 않으며, 패널도 이것들을 보여 주지 않습니다.
...
모델은 한 번 주입된 지시를 이후 모든 중단마다 전달되는 상시 지시로 바꿔 놓게 됩니다. 그래서 문구는 Windows 대시보드에서, 그 문구가
...
* **이어서 하기 메시지**: 이어서 하기 언어와 메시지 스타일. *직접 입력*에서는 어느 저장된 메시지를 쓰는지와 그 내용을 읽기
  전용으로 보여 주고, 직접 입력하는 메시지는 Windows 대시보드에서 쓴다는 안내를 붙입니다. 페이지에는 그것을 위한 텍스트 칸이 없고,
...
| | |
| --- | --- |
| **어디서** | URL 형태 하나뿐이며, `scripts/release.json`과 이 플러그인 자신의 매니페스트에 적힌 버전으로 만듭니다. "latest"도 없고, URL에 닿는 파라미터도 없습니다. 평소 실행에서 특정 버전의 플러그인은 그 버전의 압축 파일과 그 옆에 게시된 `.sha256`만 내려받습니다. `.sha256`은 대조할 고정 digest가 없을 때만 내려받습니다. `-Update`는 하나뿐인 예외이며, 그때 내려받는 버전도 입력이 아니라 바로 이 소유자와 저장소 아래의 리디렉션에서 읽은 정수 세 개이고, 아래의 검사는 모두 그대로 적용됩니다. v0.6.5부터는 요청할 때만 내려받는 것이 하나 더 있습니다. `-Compatibility`로, 또는 github.com이 답한 업데이트 확인(`-CheckOnly`, `-Update`) 뒤에 받는 Codex 호환성 데이터입니다. 고정된 주소 하나, 곧 이 저장소 `main`의 `src/codex_auto_resume/data/codex_compat.json`으로 보내는 `GET` 한 번이며, 받은 것은 설치본 자신의 검증기에 넘길 뿐 실행하지 않습니다. |
...
거부합니다. `messages.py`와 `interface.py`는 자기 테이블을 따로 두지 않고 이 카탈로그에서 문자열을 읽습니다. 번역은 저마다 그것을
...
대화 id에 묶인 스위치 *한도가 풀리면 이 작업을 자동으로 이어 가기*(또는 *이 작업을 자동으로 다시 시도하기*)가 있습니다.
