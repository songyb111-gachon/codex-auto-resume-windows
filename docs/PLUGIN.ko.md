...
아래 표는 이번 릴리스의 서버를 설명합니다. 서버는 사용자가 추가한 플러그인이 아니라 설치된 릴리스에서 실행됩니다.
v0.6.0부터 v0.6.2까지의 설치본은 tool이 열여섯 개로 `preview_recovery_message`가 없고, `update_settings`가 제시하는 것도 복구 분류,
상한, 알림뿐입니다. 언어 설정과 이어서 하기 메시지 설정은 이번 릴리스 전에는 없고, `auth_service_transient`의 스위치도 없습니다.
...
| `update_settings` | 사용자용 설정을 바꿉니다. 화면 언어, 복구 분류, 상한, 알림, 그리고 이어서 하기 메시지의 언어·스타일·적용 범위입니다. 직접 입력한 메시지 문구 자체는 아닙니다(아래 참고). | 예 |
...
그리므로, 자기만의 목록이 아니라 설정 모듈이 정의한 필드를 보여 줍니다. Codex의 밝은 테마와 어두운 테마, 그리고 움직임 줄이기
선호를 따르며, 대시보드와 팝업이 함께 쓰는 시각 언어로 그려집니다([BRAND.ko.md](BRAND.ko.md)). 위에서 아래로:
...
* **재개 대기 중**(기다리는 것이 있을 때): 작업을 최대 여덟 개까지, 각각의 코드, 사유, 다음 확인, 시도 횟수, 그리고 그 작업의
  ...
  물어볼 수 있습니다. 대시보드와 팝업의 체크박스는 동작하기 전에 정확한 중단도 확인합니다. 두 id를 함께 받는 MCP tool은 없으므로,
  패널의 스위치는 대화만 지목합니다.
* **일반**: 화면 언어.
* **자동 복구**: 곧바로 동작하는 일시 정지·재개, 복구 분류, 그리고 접혀 있는 상한.
* **알림**: 접혀 있습니다.
...
* **저장**: 위 설정 가운데 저장을 기다리는 것들을 위한 단추입니다.
...
무언가를 만들어 두지도 않았습니다. v0.5를 위해 처음부터 다시 확인했고 이번 릴리스를 위해 또 확인했으며, 답은 달라지지 않았습니다.
...
이번 릴리스를 위해 위 목록 — 검증기가 받는 필드, `apps` 항목, 대화 범위의 MCP App 화면, 닫힌 행동 버튼 집합을 가진
...
대화 id에 묶인 체크박스 *한도가 풀리면 이 작업을 자동으로 이어 가기*(또는 *이 작업을 자동으로 다시 시도하기*)가 있습니다.
